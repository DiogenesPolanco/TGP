...
export interface DashboardMetrics {
  ...
  vulnerabilities: { id: string; applicationId: string | null; severity: string; status: string; createdAt: Date }[]
  incidents: { id: string; applicationId: string | null; status: string; downtimeMinutes: number | null; createdAt: Date }[]
  risks: { id: string; applicationId: string | null; status: string; riskScore: number; createdAt: Date }[]
  auditFindings: { id: string; status: string; dueDate: Date }[]
  technologies: { id: string; supportStatus: string }[]
  teams: { id: string; currentMetrics: { deploymentFrequency: number; leadTimeHours: number } | null }[]
...
export function useDashboardMetrics(): DashboardMetrics {
  ...
  const prevStart = useMemo(() => {
    ...
    const periodDays = selectedPeriod === '7d' ? 7 : selectedPeriod === '30d' ? 30 : selectedPeriod === '90d' ? 90 : 30
  ...
  const vulnTrend: 'up' | 'down' | 'neutral' =
    criticalVulns < vulnsPrev ? 'down'
    : criticalVulns > vulnsPrev ? 'up'
    : 'neutral'
  ...
  const incidentTrend: 'up' | 'down' | 'neutral' =
    openIncidents < incidentsPrev ? 'down'
    : openIncidents > incidentsPrev ? 'up'
    : 'neutral'
