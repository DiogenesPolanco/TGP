...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden relative" style={{ background: '#080c14', color: '#c8d0e0' }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-[100]" style={{ background: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,255,136,0.015) 2px, rgba(0,255,136,0.015) 4px)' }} />

      <div className="relative z-10 max-w-[1100px] mx-auto px-6">
        <nav style={{ borderBottom: '1px solid rgba(0,255,136,0.1)' }} className="flex items-center justify-between py-6">
          <button onClick={() => navigate('/')} className="font-mono font-extrabold text-xl tracking-tight" style={{ color: '#00ff88' }}>
          ...
          <ul className="hidden md:flex gap-8 list-none text-xs font-semibold uppercase tracking-wider" style={{ color: '#6b7a99' }}>
            {[
              ...
              <li key={link.label}>
                <button onClick={() => { link.external ? window.open(link.to, '_blank', 'noopener,noreferrer') : link.scroll ? document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' }) : navigate(link.to) }}
          ...
          <button onClick={() => navigate('/dashboard')}
            ...
            style={{ background: 'transparent', border: '1px solid #00ff88', color: '#00ff88', padding: '10px 24px', borderRadius: 4 }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(0,255,136,0.1)'; e.currentTarget.style.boxShadow = '0 0 20px rgba(0,255,136,0.15)' }}
