...
export function ObjectiveFormPage() {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', type: 'okr' as Objective['type'],
    teamId: '', businessUnitId: '',
    periodStart: new Date().toISOString().split('T')[0], periodEnd: '',
    status: 'not_started' as ObjectiveStatus, keyResults: [] as KeyResult[],
  })

  useEffect(() => {
    if (objective) queueMicrotask(() => setFormData({
      title: objective.title ?? '', description: objective.description ?? '',
      type: objective.type ?? 'okr', teamId: objective.teamId ?? '',
      businessUnitId: objective.businessUnitId ?? '',
      periodStart: objective.periodStart && !isNaN(new Date(objective.periodStart).getTime())
        ? new Date(objective.periodStart).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      periodEnd: objective.periodEnd && !isNaN(new Date(objective.periodEnd).getTime())
        ? new Date(objective.periodEnd).toISOString().split('T')[0] : '',
      status: objective.status ?? 'not_started', keyResults: objective.keyResults ?? [],
    }))
  }, [objective])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const data = {
      ...formData, teamId: formData.teamId || null, businessUnitId: formData.businessUnitId || null,
      periodStart: parseLocalDate(formData.periodStart), periodEnd: parseLocalDate(formData.periodEnd),
      progress: liveProgress, metadata: objective?.metadata ?? {},
      createdAt: objective?.createdAt ?? new Date(), updatedAt: new Date(),
    }
    if (objective) { await db.objectives.update(objective.id, data); addNotification({ type: 'success', message: 'Objetivo actualizado' }) }
    else { await db.objectives.add({ ...data, id: crypto.randomUUID() }); addNotification({ type: 'success', message: 'Objetivo creado' }) }
    navigate('/strategy/objectives')
  }

  const addKeyResult = () => {
    setFormData({ ...formData, keyResults: [...formData.keyResults, { id: crypto.randomUUID(), title: '', measure: '', baseline: 0, target: 100, current: 0, status: 'not_started' }] })
    setTimeout(() => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-neutral-90 dark:text-white">{objective ? 'Editar Objetivo' : 'Nuevo Objetivo'}</h1>
          <p className="text-sm text-muted mt-0.5">{objective ? 'Modifica los datos del objetivo y sus resultados clave' : 'Define un nuevo objetivo y sus resultados clave'}</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card rounded-xl border border-boundary p-5 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Select label="Tipo" value={formData.type} onChange={(v) => setFormData({ ...formData, type: v as Objective['type'] })}
                options={[{ value: 'okr', label: 'OKR' }, { value: 'kpi', label: 'KPI' }, { value: 'balanced_scorecard', label: 'Balanced Scorecard' }]} />
              <Select label="Equipo" value={formData.teamId} onChange={(v) => setFormData({ ...formData, teamId: v })}
                options={[{ value: '', label: 'Sin equipo' }, ...teams.map((team) => ({ value: team.id, label: team.name }))]} />
              <Select label="Unidad de Negocio" value={formData.businessUnitId} onChange={(v) => setFormData({ ...formData, businessUnitId: v })}
                options={[{ value: '', label: 'Sin BU' }, ...businessUnits.map((bu) => ({ value: bu.id, label: bu.name }))]} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
              <Select label="Estado" value={formData.status} onChange={(v) => setFormData({ ...formData, status: v as ObjectiveStatus })}
                options={STATUS_OPTIONS} />
              <div>
                <label className="block text-xs font-medium text-muted mb-1.5">Inicio *</label>
                <DatePicker required value={formData.periodStart} onChange={(v) => setFormData({ ...formData, periodStart: v })}
              ...
              <div>
                ...
                <DatePicker required value={formData.periodEnd} onChange={(v) => setFormData({ ...formData, periodEnd: v })}
                  className="w-full px-3 py-2.5 rounded-lg border border-neutral-30 dark:border-neutral-60 bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
            </div>
          </div>
            ...
            {formData.keyResults.length === 0 ? (
              ...
              <div className="space-y-3">
                {formData.keyResults.map((kr, index) => (
                  <KeyResultCard key={kr.id} kr={kr} index={index} onUpdate={updateKeyResult} onRemove={removeKeyResult} />
                ))}
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1">
          <ObjectivePreviewCard
            title={formData.title} type={formData.type} status={formData.status}
            keyResults={formData.keyResults} liveProgress={liveProgress} teamId={formData.teamId} teams={teams}
          />
          <div className="space-y-2 pt-4">
            ...
            <Button type="button" onClick={() => navigate('/strategy/objectives')}
