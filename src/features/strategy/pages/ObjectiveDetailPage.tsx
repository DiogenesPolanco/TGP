...
const statusColor: Record<string, string> = {
  ...
  achieved: 'bg-info/10 text-info',
  not_started: 'bg-neutral-10 text-neutral-60',
}
...
export function ObjectiveDetailPage() {
  ...
  const team = useLiveQuery(
    () => (objective?.teamId ? db.teams.get(objective.teamId) : undefined),
    [objective],
  )
  const bu = useLiveQuery(
    () => (objective?.businessUnitId ? db.businessUnits.get(objective.businessUnitId) : undefined),
    [objective],
  )

  if (!objective) {
    return (
      <DetailLayout title="Objetivo no encontrado" onBack={() => navigate('/strategy/objectives')}>
        <p className="text-neutral-50">El objetivo no existe o ha sido eliminado.</p>
      </DetailLayout>
    )
  }
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <Section title="Período">
          <Field
            label="Inicio"
            value={new Date(objective.periodStart).toLocaleDateString('es-ES')}
          />
            ...
            return (
              <div key={kr.id} className="p-4 rounded-lg border border-boundary">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusColor[kr.status]}`}
                  >
                ...
                <div className="flex items-center gap-3 text-xs text-neutral-50 mb-2">
                  ...
                  <span>
                    Current: <strong className={krStatusColor[kr.status]}>{kr.current}</strong>
                  </span>
                  <span>Target: {kr.target}</span>
                </div>
                <div className="w-full h-1.5 bg-neutral-20 dark:bg-neutral-70 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${kr.status === 'behind' ? 'bg-danger' : kr.status === 'at_risk' ? 'bg-warning' : kr.status === 'achieved' ? 'bg-success' : 'bg-primary'}`}
