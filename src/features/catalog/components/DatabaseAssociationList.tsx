...
export function DatabaseAssociationList({ microserviceId, applicationId }: Props) {
  ...
  const allDatabases = useLiveQuery(() => db.appDatabases.toArray(), []) ?? []
          ...
          <div className="flex items-center justify-end gap-1">
            <Button
              onClick={(e) => {
                e.stopPropagation()
                navigate(
                  `/catalog/applications/${applicationId}/databases/${item.id}/edit?microserviceId=${microserviceId}`,
                )
              }}
  ...
  return (
    <div className="space-y-4">
      {associatedDatabases.length > 0 ? (
        <SortableTable columns={columns} data={associatedDatabases} pageSize={10} emptyMessage="" />
      ) : (
        <p className="text-sm text-neutral-50">
          Sin bases de datos asociadas a este microservicio.
        </p>
      )}
      ...
      <div className="flex items-center gap-3">
        <Button
          onClick={() =>
            navigate(
              `/catalog/applications/${applicationId}/databases/new?microserviceId=${microserviceId}`,
            )
          }
                ...
                {availableDatabases.length === 0 ? (
                  ...
                  availableDatabases.map((db_) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-neutral-50 shrink-0 font-mono">
