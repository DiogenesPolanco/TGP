...
export function AzureShareConfigSection({
  useSameForSharing, backupConfigured, effectiveConfigured,
  shareSasUrl, onShareSasUrlChange, shareContainerName, onShareContainerNameChange,
  showShareUrl, onToggleShowShareUrl, testingShare, shareTestResult,
  shareContainerConfigured, backupContainerName,
  onToggleUseSame, onTest, onSave, onClear,
}: Props) {
  return (
    <div className="space-y-3">
      ...
      <label className="flex items-start gap-3 cursor-pointer group">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-neutral-50 mt-0.5">
            Los enlaces compartidos se almacenarán en el mismo contenedor de backup. Recomendado para simplicidad.
          </p>
        </div>
      </label>
        ...
        <Card padding={false} className="p-4 space-y-3">
          <div className="flex items-start gap-2 p-3 rounded-lg bg-info/5 text-info text-xs">
            ...
            <span>
              ...
              <code className="px-1 py-0.5 rounded bg-neutral-20 dark:bg-neutral-70 text-[10px]">rc</code>
              ) en la SAS URL.
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" onClick={onTest} isLoading={testingShare} leftIcon={<RefreshCw size={14} />}>
          ...
          {shareTestResult && (
            <div className={`flex items-start gap-2 p-3 rounded-lg text-sm ${shareTestResult.success ? 'bg-success/5 text-success' : 'bg-danger/5 text-danger'}`}>
          ...
          {shareContainerConfigured && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-info/5 text-info text-xs">
              <Info size={14} className="mt-0.5 shrink-0" />
              <span>
                Compartiendo en <strong>{shareContainerConfigured}</strong>. Los datos se cifran (AES-GCM 256) antes de subir. Archivos expirados se limpian automáticamente.
              </span>
            </div>
          )}
        </Card>
      ...
      {useSameForSharing && backupConfigured && (
        <div className="flex items-start gap-2 p-3 rounded-lg bg-info/5 text-info text-xs">
          <Info size={14} className="mt-0.5 shrink-0" />
          <span>
            Los enlaces públicos se almacenan en <strong>{backupContainerName}</strong> (mismo contenedor que backup). Siempre se cifran (AES-GCM 256) antes de subir. Los archivos expirados se limpian automáticamente.
          </span>
        </div>
      )}
