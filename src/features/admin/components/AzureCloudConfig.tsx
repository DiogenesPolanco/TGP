...
export function AzureCloudConfig() {
  ...
  useEffect(() => {
    if (initialInfo.configured) startTransition(() => loadBackupList())
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      addNotification({
        type: 'success',
        message: `Backup subido: ${blobName} (${Math.round(sizeBytes / 1024)} KB)`,
      })
      loadBackupList()
    } catch (err) {
      addNotification({
  ...
  const handleToggleUseSame = (same: boolean) => {
    ...
    } else if (!savedShareConfig && info.configured) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {info.configured && (
          <Badge color="success">
            <CheckCircle2 size={12} className="mr-1" /> Configurado
          </Badge>
        )}
      </div>
      ...
      <AzureShareConfigSection
        useSameForSharing={useSameForSharing}
        backupConfigured={info.configured}
        effectiveConfigured={effectiveShareConfigured}
        shareSasUrl={shareSasUrl}
        onShareSasUrlChange={setShareSasUrl}
        shareContainerName={shareContainerName}
        onShareContainerNameChange={setShareContainerName}
        showShareUrl={showShareUrl}
        onToggleShowShareUrl={() => setShowShareUrl(!showShareUrl)}
        testingShare={testingShare}
        shareTestResult={shareTestResult}
        ...
        onTest={handleTestShare}
        onSave={handleSaveShare}
        onClear={handleClearShare}
      />
