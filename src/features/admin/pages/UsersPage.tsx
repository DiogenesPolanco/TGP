  ...
  const columns: Column<User>[] = [
    {
      ...
      sortable: true,
      render: (u) => (
        <span className="text-sm font-medium text-neutral-90 dark:text-white">{u.displayName}</span>
      ),
    },
    {
      key: 'email',
      label: 'Email',
      sortable: true,
      render: (u) => <span className="text-sm text-secondary">{u.email}</span>,
    },
    {
      key: 'role',
      label: 'Rol',
      ...
      render: (u) => {
        ...
        return (
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${colors[u.role] || 'bg-neutral-10 text-neutral-60'}`}
          >
    ...
    },
    {
      ...
      render: (u) => (
        <span
          className={`text-xs px-2 py-0.5 rounded-full ${u.isActive ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}
        >
