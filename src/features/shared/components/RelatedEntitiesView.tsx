    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <EntitySection
          ...
          icon={<Server size={16} />}
          color="primary"
          empty="Sin aplicaciones vinculadas"
        >
          {data.apps.map((app) => (
            <EntityCard
              key={app.id}
        ...
        <EntitySection
          ...
          icon={<Box size={16} />}
          color="info"
          empty="Sin microservicios vinculados"
        >
        ...
        <EntitySection
          ...
          icon={<Shield size={16} />}
          color={counts.vulns > 0 ? 'danger' : 'neutral'}
          empty="Sin vulnerabilidades"
        >
          {data.vulns.map((v) => (
            <SimpleCard
              key={v.id}
              title={v.title}
              subtitle={severityLabel[v.severity] ?? v.severity}
              onClick={() => navigate(`/security/vulnerabilities/${v.id}`)}
            />
          ))}
        </EntitySection>
        ...
        <EntitySection
          ...
          icon={<Activity size={16} />}
          color={counts.incidents > 0 ? 'danger' : 'neutral'}
          empty="Sin incidentes"
        >
          {data.incidents.map((i) => (
            <SimpleCard
              key={i.id}
              title={i.title}
              subtitle={severityLabel[i.severity] ?? i.severity}
              onClick={() => navigate(`/security/incidents/${i.id}`)}
            />
          ))}
        </EntitySection>
        ...
        >
          {data.risks.map((r) => (
            <SimpleCard
              key={r.id}
              title={r.title}
              subtitle={`Score: ${r.riskScore}`}
              onClick={() => navigate(`/governance/risks/${r.id}`)}
            />
          ))}
        </EntitySection>
        ...
        >
          {data.auditFindings.map((a) => (
            <SimpleCard
              key={a.id}
              title={a.title}
              subtitle={severityLabel[a.severity] ?? a.severity}
              onClick={() => navigate(`/governance/audit/${a.id}`)}
            />
          ))}
        </EntitySection>
      </div>
...
function StatBadge({
  ...
  return (
    <div
      className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-lg border border-boundary bg-white dark:bg-neutral-85`}
    >
      <div className={`p-1.5 rounded-md ${colorMap[color] || colorMap.neutral}`}>{icon}</div>
      <div>
        <p className="text-lg font-bold text-neutral-90 dark:text-white tabular-nums leading-none">
          {value}
        </p>
...
function SimpleCard({
  ...
  const content = (
    <>
      <span
        className={`text-sm truncate min-w-0 flex-1 ${onClick ? 'group-hover:text-primary transition-colors' : 'text-neutral-90 dark:text-white'}`}
      >
        {title}
      </span>
