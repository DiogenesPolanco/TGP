    ...
    <div className="space-y-6">
        ...
        <div className="bg-card rounded-xl border border-boundary p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-lg font-bold text-neutral-90 dark:text-white">
              {
                new Set([...deps.map((d) => d.applicationId), ...deps.map((d) => d.dependsOnAppId)])
                  .size
              }
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {nodes.length > 0 ? (
              ...
              <div className="text-center py-16 text-neutral-50">
                ...
                <p className="text-sm mt-1">
                  Las dependencias aparecerán aquí cuando añadas aplicaciones y sus relaciones
                </p>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {edges.length > 0 && (
            <div className="bg-card rounded-xl border border-boundary p-4">
              <h3 className="text-sm font-bold text-neutral-90 dark:text-white mb-3">
                Dependencias
              </h3>
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {deps.map((dep) => {
                  ...
                  return (
                    <div
                      key={dep.id}
                      className="flex items-center gap-2 text-xs p-2 rounded-lg hover:bg-neutral-5 dark:hover:bg-neutral-85 transition-colors"
                    >
                      <span className="font-medium text-neutral-90 dark:text-white truncate">
                        {sourceApp?.name ?? dep.applicationId}
                      </span>
                      <span className="text-neutral-40">→</span>
                      <span className="font-medium text-neutral-90 dark:text-white truncate">
                        {targetApp?.name ?? dep.dependsOnAppId}
                      </span>
                      <span
