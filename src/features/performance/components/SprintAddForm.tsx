...
export function SprintAddForm({
  sprintName, quarter, year, storyPointsCompleted, storyPointsNotCompleted,
  loadingTeamSprints, sprintSelectOptions, teamSprints,
  onChange, onSave, onCancel, onSelectTeamSprint,
}: Props) {
    ...
    <div className="mb-6 p-4 border border-boundary rounded-lg bg-neutral-10 dark:bg-neutral-70">
      <div className="grid gap-3 sm:grid-cols-5 mb-3">
        ...
        <div>
          ...
          <Select
            value={quarter}
            onChange={(v) => onChange({ sprintName, quarter: v, year, storyPointsCompleted, storyPointsNotCompleted })}
            options={[
        ...
        <div>
          ...
          <input
            type="number"
            value={year}
            onChange={(e) => onChange({ sprintName, quarter, year: Number(e.target.value), storyPointsCompleted, storyPointsNotCompleted })}
            className={inputClass}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ sprintName, quarter, year, storyPointsCompleted, storyPointsNotCompleted: Number(e.target.value) })}
            className={inputClass}
            min={0}
          />
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-3">
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ sprintName, quarter, year, storyPointsCompleted: Number(e.target.value), storyPointsNotCompleted })}
            className={inputClass}
            min={0}
          />
        </div>
      ...
      <div className="flex gap-2">
        <Button onClick={onSave} className="px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-dark">
          Guardar Sprint
        </Button>
        <Button onClick={onCancel} variant="ghost" className="px-4 py-2 text-sm text-neutral-60 hover:text-neutral-90">
          Cancelar
        </Button>
      </div>
    </div>
