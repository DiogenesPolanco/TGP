...
interface Props { memberId: string; teamId: string }
...
export function SprintsSection({ memberId, teamId }: Props) {
  ...
  const [newSprint, setNewSprint] = useState({ sprintName: '', quarter: 'Q2', year: new Date().getFullYear(), storyPointsCompleted: 0, storyPointsNotCompleted: 0 })
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editData, setEditData] = useState({ sprintName: '', quarter: 'Q1', year: new Date().getFullYear(), storyPointsCompleted: 0, storyPointsNotCompleted: 0 })

  useEffect(() => { db.sprintRecords.where('memberId').equals(memberId).toArray().then(setSprints) }, [memberId])
  useEffect(() => {
    db.teamSprints.where('teamId').equals(teamId).toArray().then((data) => { setTeamSprints(data); setLoadingTeamSprints(false) })
  }, [teamId])

  const availableYears = useMemo(() => Array.from(new Set(sprints.map((s) => s.year))).sort((a, b) => b - a), [sprints])
  const filteredSprints = useMemo(() => sprints.filter((s) => (yearFilter === 'all' || s.year === yearFilter) && (quarterFilter === 'all' || s.quarter === quarterFilter)), [sprints, yearFilter, quarterFilter])

  const chartData = useMemo(() =>
    [...filteredSprints].sort((a, b) => a.sprintName.toLowerCase().localeCompare(b.sprintName.toLowerCase()))
      .map((s) => ({
        ...
        eficiencia: s.storyPointsCompleted + s.storyPointsNotCompleted > 0
          ? Math.round((s.storyPointsCompleted / (s.storyPointsCompleted + s.storyPointsNotCompleted)) * 100) : 0,
      })), [filteredSprints])

  const handleTeamSprintSelect = (sprintName: string, setter: typeof setNewSprint | typeof setEditData, current: typeof newSprint) => {
    const ts = teamSprints.find((s) => s.sprintName === sprintName)
    setter({ ...current, sprintName, quarter: ts?.quarter ?? current.quarter, year: ts?.year ?? current.year })
  }

  const addSprint = async () => {
    if (!newSprint.sprintName.trim()) return
    const record: SprintRecord = { id: crypto.randomUUID(), memberId, ...newSprint, createdAt: new Date() }
  ...
  const startEdit = (sp: SprintRecord) => {
    setEditingId(sp.id)
    setEditData({ sprintName: sp.sprintName, quarter: sp.quarter, year: sp.year, storyPointsCompleted: sp.storyPointsCompleted, storyPointsNotCompleted: sp.storyPointsNotCompleted })
  }

  const saveEdit = async () => {
    if (!editingId || !editData.sprintName.trim()) return
    const updated = sprints.map((sp) => sp.id === editingId ? { ...sp, ...editData, sprintName: editData.sprintName.trim() } : sp)
  ...
  const handleEditField = (field: string, value: string | number) => setEditData({ ...editData, [field]: value })
  const handleEditSprintSelect = (sprintName: string) => handleTeamSprintSelect(sprintName, setEditData, editData)
  ...
  const sprintSelectOptions = useMemo(() => {
    ...
    return teamSprints.filter((ts) => !existing.has(ts.sprintName)).sort((a, b) => {
    ...
    <div className="bg-card rounded-xl border border-boundary p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-neutral-10 dark:bg-neutral-70 rounded-lg p-0.5">
            <button onClick={() => setViewMode('list')} className="p-2 rounded hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-muted hover:text-neutral-90 dark:hover:text-white"><List size={16} /></button>
            <button onClick={() => setViewMode('chart')} className="p-2 rounded hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-muted hover:text-neutral-90 dark:hover:text-white"><BarChart3 size={16} /></button>
          </div>
          <Button onClick={() => setShowForm(true)} variant="ghost" className="flex items-center gap-1"><Plus size={16} /> Agregar Sprint</Button>
        </div>
      </div>
      ...
      {sprints.length > 0 && (
        <div className="flex items-center gap-3 mb-4">
          <Select value={typeof yearFilter === 'number' ? String(yearFilter) : 'all'} onChange={(v) => setYearFilter(v === 'all' ? 'all' : Number(v))}
            options={[{ value: 'all', label: 'Todos los años' }, ...availableYears.map((y) => ({ value: String(y), label: String(y) }))]} />
          <Select value={quarterFilter} onChange={(v) => setQuarterFilter(v)}
            options={[{ value: 'all', label: 'Todos los quarters' }, { value: 'Q1', label: 'Q1' }, { value: 'Q2', label: 'Q2' }, { value: 'Q3', label: 'Q3' }, { value: 'Q4', label: 'Q4' }]} />
        </div>
      )}
      ...
      {showForm && (
        <SprintAddForm
          sprintName={newSprint.sprintName} quarter={newSprint.quarter} year={newSprint.year}
          storyPointsCompleted={newSprint.storyPointsCompleted} storyPointsNotCompleted={newSprint.storyPointsNotCompleted}
          loadingTeamSprints={loadingTeamSprints} sprintSelectOptions={sprintSelectOptions} teamSprints={teamSprints}
          onChange={(d) => setNewSprint({ ...newSprint, ...d })}
          onSelectTeamSprint={(name) => handleTeamSprintSelect(name, setNewSprint, newSprint)}
          onSave={addSprint} onCancel={() => setShowForm(false)}
        />
      )}
