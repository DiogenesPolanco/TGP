...
import type { MemberMicroservice } from '@/types/domain'
import {
...
export function MicroservicesSection({ memberId }: Props) {
  const [profile, setProfile] = useState<{ microservices: MemberMicroservice[] } | null>(null)
  const [microservices, setMicroservices] = useState<MemberMicroservice[]>([])
  ...
  useEffect(() => {
    Promise.all([
      ...
      setProfile(p ?? null)
      setMicroservices((p as any)?.microservices ?? [])
      setAllTechnologies(techs)
    })
  }, [memberId])

  const saveMicroservices = async (updated: MemberMicroservice[]) => {
    ...
    await db.memberProfiles.put({
      ...
      phoneCell: existing.phoneCell ?? '',
      phoneHome: existing.phoneHome ?? '',
      address: existing.address ?? '',
      role: existing.role ?? 'developer',
      skills: existing.skills ?? [],
      technologies: existing.technologies ?? [],
      avgStoryPoints: existing.avgStoryPoints ?? 0,
      vacationDaysPerYear: existing.vacationDaysPerYear ?? 20,
      vacationUsed: existing.vacationUsed ?? 0,
  ...
  const handleDelete = async (msId: string) => {
    ...
    await saveMicroservices(microservices.filter((m) => m.id !== msId))
  }

  const handleSave = async (data: { id?: string; name: string; description: string; technologies: string[] }) => {
    if (editingId) {
      await saveMicroservices(
        microservices.map((m) => m.id === editingId ? { ...m, ...data } : m)
      )
    } else {
      await saveMicroservices([
        ...microservices,
        { id: crypto.randomUUID(), ...data } as MemberMicroservice,
      ])
    }
    setShowForm(false)
    setEditingId(null)
    ...
    <div>
      ...
      <div className="space-y-3">
        {microservices.length === 0 && (
          <p className="text-sm text-neutral-50 dark:text-neutral-50">
            No hay microservicios registrados para este miembro.
          </p>
        )}
        ...
        {microservices.map((ms) => (
          <MicroserviceCard
            ...
            onEdit={() => { setEditingId(ms.id); setShowForm(true) }}
      ...
      {showForm && (
        <MicroserviceFormModal
          editingMs={editingId ? microservices.find((m) => m.id === editingId) ?? null : null}
          allTechnologies={allTechnologies}
          onSave={handleSave}
...
function MicroserviceFormModal({
  ...
  editingMs: { id: string; name: string; description: string; technologies: string[] } | null
  allTechnologies: Technology[]
  onSave: (data: { id?: string; name: string; description: string; technologies: string[] }) => void
  onClose: () => void
}) {
  const [name, setName] = useState(editingMs?.name ?? '')
  const [description, setDescription] = useState(editingMs?.description ?? '')
  const [selectedTechIds, setSelectedTechIds] = useState<string[]>(editingMs?.technologies ?? [])
      ...
      <div className="w-full max-w-lg bg-white dark:bg-neutral-80 rounded-2xl border border-neutral-20 dark:border-neutral-70 shadow-2xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-neutral-90 dark:text-white">
            {editingMs ? 'Editar Microservicio' : 'Nuevo Microservicio'}
          </h3>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            onClick={() => onSave({ id: editingMs?.id, name: name.trim(), description: description.trim(), technologies: selectedTechIds })}
            disabled={!name.trim()}
          ...
          >
            {editingMs ? 'Actualizar' : 'Crear Microservicio'}
          </button>
        </div>
      </div>
    </div>
