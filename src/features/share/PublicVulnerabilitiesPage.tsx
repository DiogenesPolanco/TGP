...
const sevColors: Record<string, string> = {
  ...
  low: 'text-neutral-60 bg-neutral-10 dark:bg-neutral-70',
}

export function PublicVulnerabilitiesPage() {
  const { hash } = useParams<{ hash: string }>()
  const { loading, valid, data, pendingEncrypted, handleDecrypt } = usePublicShare(
    hash,
    () => getPublicVulnerabilitiesData() as Promise<PublicVulnerabilitiesData>,
  )
  ...
  const columns: Column<PublicVulnerabilitiesData['vulnerabilities'][number] & { id: string }>[] = [
    ...
    { key: 'cvssScore', label: 'CVSS', sortable: true, className: 'text-right', render: (v) => <span className="text-neutral-60 dark:text-neutral-40">{v.cvssScore?.toFixed(1) ?? '—'}</span> },
    {
      key: 'applicationId',
      label: 'Aplicación',
      sortable: true,
    ...
    if (pendingEncrypted) {
      return (
        <div className="min-h-screen bg-neutral-10 dark:bg-neutral-90 flex items-center justify-center">
          <PassphraseModal
            title="Vulnerabilidades protegidas"
    ...
    <div id="printable-content" className="min-h-screen bg-neutral-10 dark:bg-neutral-90">
      ...
      <main className="px-6 py-8 max-w-screen-2xl mx-auto">
        <div className="bg-white dark:bg-neutral-80 rounded-2xl border border-neutral-20 dark:border-neutral-70 shadow-sm overflow-hidden">
          ...
          <SortableTable
            columns={columns}
            data={data.vulnerabilities.map((v) => ({ ...v, id: v.id }))}
            pageSize={25}
          />
        </div>
