  ...
  const analytics = useMemo(() => {
    ...
    const membersWithStats = data.members.map((m) => {
      ...
      const avgMood = oneOnOnes.length
        ? Math.round((oneOnOnes.reduce((s, o) => s + o.estadoAnimo, 0) / oneOnOnes.length) * 10) /
          10
        : 0
      ...
      return {
        ...
        avgCompletion,
        avgMood,
        sprintCount: msprints.length,
        oneOnOneCount: oneOnOnes.length,
      }
    })

    const sortedBySP = [...membersWithStats].sort((a, b) => b.totalSP - a.totalSP)
    const sortedByCompletion = [...membersWithStats].sort(
      (a, b) => b.avgCompletion - a.avgCompletion,
    )
    const sortedByMood = [...membersWithStats].sort((a, b) => b.avgMood - a.avgMood)
    const needsAttention = membersWithStats.filter(
      (m) => (m.avgMood > 0 && m.avgMood < 3) || (m.avgCompletion > 0 && m.avgCompletion < 40),
    )

    const avgSP = membersWithStats.length
      ? Math.round(membersWithStats.reduce((s, m) => s + m.totalSP, 0) / membersWithStats.length)
      : 0
    const avgCompl = membersWithStats.length
      ? Math.round(
          membersWithStats.reduce((s, m) => s + m.avgCompletion, 0) / membersWithStats.length,
        )
      : 0
    const avgMoodAll = membersWithStats.filter((m) => m.avgMood > 0).length
      ? Math.round(
          (membersWithStats.filter((m) => m.avgMood > 0).reduce((s, m) => s + m.avgMood, 0) /
            membersWithStats.filter((m) => m.avgMood > 0).length) *
            10,
        ) / 10
      : 0
    ...
    <div id="printable-content" className="min-h-screen bg-canvas">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          <SummaryCard
            label="Miembros"
            value={analytics.all.length}
            icon={<Users size={18} />}
            color="primary"
          />
          <SummaryCard
            label="SP promedio"
            value={analytics.avgSP}
            icon={<BarChart3 size={18} />}
            color="info"
          />
          <SummaryCard
            label="Completitud prom."
            value={`${analytics.avgCompl}%`}
            icon={<TrendingUp size={18} />}
            color={analytics.avgCompl >= 70 ? 'success' : 'warning'}
          />
          <SummaryCard
            label="Ánimo promedio"
            value={analytics.avgMoodAll > 0 ? `${analytics.avgMoodAll}/5` : '—'}
            icon={<Brain size={18} />}
            color={
              analytics.avgMoodAll >= 4
                ? 'success'
                : analytics.avgMoodAll >= 3
                  ? 'warning'
                  : 'danger'
            }
          />
          ...
          >
            {analytics.topSP.map((item, i) => (
              <MemberRow
                ...
                rightLabel={`${item.totalSP} SP`}
                rightSublabel={`${item.sprintCount} sprints`}
                highlight={i === 0}
              />
            ))}
          </CategoryCard>
          ...
          >
            {analytics.topCompletion.map((item, i) => (
              <MemberRow
                ...
                rank={i + 1}
                name={item.displayName}
                rightLabel={`${item.avgCompletion}%`}
                rightSublabel={`${item.totalSP} SP totales`}
                highlight={i === 0}
              />
            ))}
          </CategoryCard>
            ...
            {analytics.topMood
              ...
              .map((item, i) => (
                <MemberRow
                  ...
                  rank={i + 1}
                  name={item.displayName}
                  rightLabel={`${item.avgMood}/5`}
                  rightSublabel={`${item.oneOnOneCount} reuniones`}
                  highlight={i === 0}
                />
              ))}
        ...
        {analytics.needsAttention.length > 0 && (
          <div className="bg-card rounded-2xl border border-danger/20 dark:border-danger/30 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-boundary flex items-center gap-2">
              ...
              <span className="text-xs text-danger/70 ml-auto">
                {analytics.needsAttention.length} miembro
                {analytics.needsAttention.length > 1 ? 's' : ''}
              </span>
            </div>
            <div className="divide-y divide-neutral-10 dark:divide-neutral-75">
              {analytics.needsAttention.map((item) => {
                const reasons: string[] = []
                if (item.avgCompletion > 0 && item.avgCompletion < 40)
                  reasons.push(`Baja completitud (${item.avgCompletion}%)`)
                if (item.avgMood > 0 && item.avgMood < 3)
                  reasons.push(`Ánimo bajo (${item.avgMood}/5)`)
                return (
          ...
          <div className="space-y-3">
            {analytics.all
              .map((item) => ({
                ...item,
                _score: Math.round(
                  item.avgCompletion * 0.6 + Math.min(item.totalSP / 5, 100) * 0.4,
                ),
              }))
              ...
              .map((item) => (
                <div key={item.member.id} className="flex items-center gap-4">
                  <span className="text-sm text-secondary font-medium w-32 truncate">
                    {item.displayName}
                  </span>
                  ...
                  <span className="text-sm font-semibold text-neutral-90 dark:text-white w-16 text-right">
                    {item._score}%
                  </span>
