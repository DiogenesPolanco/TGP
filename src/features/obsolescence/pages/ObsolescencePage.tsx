...
export function ObsolescencePage() {
  ...
  const filteredTechs = useMemo(() => technologies.filter((t) => {
  ...
  const appTechMap = useMemo(() => computeAppTechMap(applications, microservices), [applications, microservices])
  ...
  const stats = useMemo(() => {
    ...
    const criticalAppsWithEol = applications.filter((app) => {
      ...
      return allTechIds.some((tId) => eolTechIds.has(tId)) && (app.criticality === 'critical' || app.criticality === 'high')
    }).length
    const sixMonthsFromNow = new Date()
    sixMonthsFromNow.setMonth(sixMonthsFromNow.getMonth() + 6)
    const nearEol = technologies.filter((t) => t.eolDate && new Date(t.eolDate) <= sixMonthsFromNow && new Date(t.eolDate) > new Date()).length
  ...
  const handleDelete = async (id: string) => {
    ...
    for (const app of affectedApps) {
      await db.applications.update(app.id, { technologies: app.technologies.filter((tId) => tId !== id) })
    }
    await db.technologies.delete(id)
  }

  const categories = useMemo(() => Array.from(new Set(technologies.map((t) => t.category))) as TechCategory[], [technologies])
  const categoryOptions = categories.map((cat) => ({ value: cat, label: categoryLabels[cat] || cat }))

  const columns = useObsColumns({ technologies, applications, microservices, appTechMap, onNavigate: (path) => navigate(path), onDelete: handleDelete })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={() => navigate('/admin/import')} variant="secondary" size="sm"><Upload size={16} /> Importar</Button>
          <Button onClick={() => navigate('map')} variant="secondary" size="sm"><Network size={16} /> Mapa de Obsolescencias</Button>
          <Button onClick={() => navigate('new')} variant="primary" size="md" className="bg-primary text-white hover:bg-primary/90"><Plus size={18} /> Nueva Tecnología</Button>
        </div>
      </div>

      <ObsolescenceStatsCards stats={stats} onFilterStatus={(s) => setStatusFilter(s as SupportStatus | 'all')} onReset={() => { setStatusFilter('all'); setSearch('') }} />
      ...
      <div className="bg-card rounded-2xl border border-boundary p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input type="text" placeholder="Buscar por nombre o vendor..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 bg-transparent" />
          </div>
          <Select value={statusFilter} onChange={(v) => setStatusFilter(v as SupportStatus | 'all')}
            options={[{ value: 'all', label: 'Todos los estados' }, { value: 'active', label: 'Activo' }, { value: 'extended', label: 'Soporte Extendido' }, { value: 'eol', label: 'EOL' }, { value: 'unknown', label: 'Desconocido' }]}
            className="min-w-[180px]" />
          <Select value={categoryFilter} onChange={(v) => setCategoryFilter(v as TechCategory | 'all')}
            options={[{ value: 'all', label: 'Todas las categorías' }, ...categoryOptions]}
            className="min-w-[180px]" />
          {(search || statusFilter !== 'all' || categoryFilter !== 'all') && (
            <Button onClick={() => { setSearch(''); setStatusFilter('all'); setCategoryFilter('all') }} variant="ghost" size="sm">Limpiar filtros</Button>
          )}
        </div>
      </div>

      <SortableTable columns={columns} data={filteredTechs} onRowClick={(tech) => navigate(`${tech.id}`)} pageSize={5}
        emptyMessage={technologies.length === 0 ? 'No hay tecnologías registradas. Crea la primera.' : 'No se encontraron tecnologías con los filtros seleccionados.'}
      />
    </div>
  )
}
