...
export function ObsolescenceMapPage() {
  ...
  const allNodes = useMemo(() => {
    const result: { id: string; label: string; status: SupportStatus | 'mixed'; type: 'app' | 'microservice'; parentAppId?: string; techCount: number; appName: string }[] = []
    for (const app of apps) {
      const techIds = appTechMap.get(app.id) ?? app.technologies
      result.push({ id: app.id, label: app.name, status: getWorstStatus(techIds, techMap), type: 'app', techCount: techIds.length, appName: app.name })
    }
    for (const ms of microservices) {
      const techIds = msTechMap.get(ms.id) ?? []
      const parentApp = apps.find((a) => a.id === ms.applicationId)
      result.push({ id: ms.id, label: ms.name, status: getWorstStatus(techIds, techMap), type: 'microservice', parentAppId: ms.applicationId, techCount: techIds.length, appName: parentApp?.name ?? '' })
    }
    return result
  }, [apps, microservices, appTechMap, msTechMap, techMap])

  const allEdges = useMemo(() => microservices.map((ms) => ({ source: ms.applicationId, target: ms.id })), [microservices])
  ...
  const filteredEdges = useMemo(() => allEdges.filter((e) => filteredNodeIds.has(e.source) && filteredNodeIds.has(e.target)), [allEdges, filteredNodeIds])

  const stats = useMemo(() => {
    const appNodes = allNodes.filter((n) => n.type === 'app')
    return {
      totalApps: apps.length, totalMs: microservices.length,
  ...
  const handleNodeClick = useCallback((nodeId: string, nodeType: 'app' | 'microservice', parentAppId?: string) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Button onClick={() => navigate('/catalog/obsolescence')} className="p-2 rounded-lg hover:bg-neutral-10 dark:hover:bg-neutral-70 transition-colors">
          ...
          <div>
            ...
            <p className="text-sm text-muted">Apps y microservicios coloreados por estado de soporte de sus tecnologías</p>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        <StatCard icon={<Layers size={18} />} value={stats.totalApps} label="Aplicaciones" iconBg="bg-primary/10 text-primary"
          active={statusFilter === 'all' && showMicroservices && !search}
          onClick={() => { setStatusFilter('all'); setShowMicroservices(true); setSearch('') }} />
        <StatCard icon={<Network size={18} />} value={stats.totalMs} label="Microservicios" iconBg="bg-info/10 text-info"
          active={showMicroservices && statusFilter === 'all' && !search}
          onClick={() => setShowMicroservices(!showMicroservices)} />
        <StatCard icon={<AlertTriangle size={18} />} value={stats.appsWithEol} label="Apps con EOL" iconBg="bg-danger/10 text-danger"
          valueColor="text-danger" active={statusFilter === 'eol'}
          onClick={() => setStatusFilter(statusFilter === 'eol' ? 'all' : 'eol')} />
        <StatCard icon={<Clock size={18} />} value={stats.appsWithExtended} label="Soporte Extendido" iconBg="bg-warning/10 text-warning"
          valueColor="text-warning" active={statusFilter === 'extended'}
          onClick={() => setStatusFilter(statusFilter === 'extended' ? 'all' : 'extended')} />
        <StatCard icon={<Info size={18} />} value={stats.appsWithMixed} label="Estado Mixto" iconBg="bg-purple-500/10 text-purple-500"
          valueColor="text-purple-500" active={statusFilter === 'mixed'}
          onClick={() => setStatusFilter(statusFilter === 'mixed' ? 'all' : 'mixed')} />
        <StatCard icon={<CheckCircle size={18} />} value={stats.appsActive} label="Apps Saludables" iconBg="bg-success/10 text-success"
          valueColor="text-success" active={statusFilter === 'active'}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 bg-card rounded-2xl border border-boundary shadow-sm overflow-hidden">
          <div className="p-3 border-b border-boundary flex items-center justify-between">
            ...
            <span className="text-xs text-neutral-50">{filteredNodes.length} nodos · {filteredEdges.length} conexiones</span>
          </div>
          <div className="p-3">
            {filteredNodes.length > 0 ? (
              <div className="w-full overflow-auto" style={{ minHeight: 500 }}>
                <ObsolescenceGraph nodes={filteredNodes} edges={filteredEdges} width={900} height={600} onNodeClick={handleNodeClick} />
              </div>
            ) : (
        ...
        <ObsolescenceMapSidebar
          ...
          showMicroservices={showMicroservices} onToggleMicroservices={() => setShowMicroservices(!showMicroservices)}
