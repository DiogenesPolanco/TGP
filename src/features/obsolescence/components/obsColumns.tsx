...
import { getStatusLabel, getStatusStyle, getEolUrgency, getLifecyclePct, categoryLabels } from '../utils/obsolescenceHelpers'
...
export function useObsColumns({ applications, appTechMap, onNavigate, onDelete }: ObsColumnsProps) {
  const columns: Column<Technology>[] = [
    {
      key: 'name', label: 'Tecnología', sortable: true,
    ...
    {
      key: 'supportStatus', label: 'Ciclo de Vida', sortable: true,
      render: (tech) => {
    ...
    {
      key: 'eolDate', label: 'EOL Date', sortable: true,
      render: (tech) => {
        const urgency = getEolUrgency(tech)
            ...
            {eol && (
              <div className="space-y-0.5">
                <div className="h-1.5 bg-neutral-10 dark:bg-neutral-85 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${expired ? 'bg-danger' : remainingDays! < 180 ? 'bg-warning' : remainingDays! < 365 ? 'bg-severity-high' : 'bg-success'}`}
    ...
    {
      key: 'apps', label: 'Apps', headerClassName: 'text-center', className: 'text-center',
      render: (tech) => {
    ...
    {
      key: 'cveList', label: 'CVEs', headerClassName: 'text-center', className: 'text-center',
      render: (tech) => (
    ...
    },
    {
      ...
      render: (tech) => (
        <div className="flex items-center justify-end gap-1">
          <Button onClick={(e) => { e.stopPropagation(); onNavigate(`${tech.id}/edit`) }} variant="ghost" size="sm" className="p-1.5" title="Editar"><Pencil size={16} /></Button>
          <Button onClick={(e) => { e.stopPropagation(); onDelete(tech.id) }} variant="ghost" size="sm" className="p-1.5 hover:text-danger" title="Eliminar"><Trash2 size={16} /></Button>
        </div>
      ),
    },
  ]
