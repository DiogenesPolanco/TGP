    ...
    >
      ...
      {activeTab === 'info' && (
        <div className="space-y-6">
          {/* Hero banner */}
          <div className={`rounded-xl border p-5 ${severityBg[finding.severity] || 'bg-neutral-10'}`}>
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl ${severityIconBg[finding.severity] || 'bg-neutral-40'} flex items-center justify-center shadow-sm`}>
                  {severityIcon[finding.severity] || <HelpCircle size={24} className="text-white" />}
                </div>
                <div>
                  <p className="text-xs font-medium uppercase tracking-wider text-neutral-50 dark:text-neutral-40">Severidad</p>
                  <p className="text-xl font-bold text-neutral-90 dark:text-white">{severityLabel[finding.severity] ?? finding.severity}</p>
                  <p className="text-sm text-neutral-60 dark:text-neutral-40 mt-0.5">
                    {categoryLabel[finding.category] ?? finding.category} · {statusLabel[finding.status] ?? finding.status}
                  </p>
                </div>
              </div>
              <div className={`px-4 py-2 rounded-lg ${slaColor} text-center`}>
                <p className="text-2xl font-bold tabular-nums">{slaDays < 0 ? Math.abs(slaDays) : slaDays}</p>
                <p className="text-[10px] uppercase tracking-wider">{slaDays < 0 ? 'días vencido' : 'días restantes'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Section title="Información General" icon={<Server size={18} />}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <MiniField label="Categoría" value={categoryLabel[finding.category] ?? finding.category} />
            ...
            <Section title="Clasificación" icon={<Shield size={18} />}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <MiniField
                  ...
                  value={
                    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${
                      ...
                      finding.status === 'in_progress' ? 'bg-warning/10 text-warning' :
                ...
                />
                <MiniField label="Vencimiento" value={new Date(finding.dueDate).toLocaleDateString('es-ES', { year: 'numeric', month: 'short', day: 'numeric' })} />
                <MiniField
                  label="SLA"
                  value={
                    <span className={`text-xs font-medium ${slaDays < 0 ? 'text-danger' : slaDays <= 7 ? 'text-warning' : 'text-success'}`}>
          ...
          {finding.description && (
            <Section title="Descripción" icon={<FileWarning size={18} />}>
              <p className="text-sm text-neutral-70 dark:text-neutral-30 leading-relaxed">{finding.description}</p>
            </Section>
          )}
