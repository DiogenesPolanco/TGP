...
export function RiskFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (risk) {
      ...
      addNotification({ type: 'success', message: 'Riesgo actualizado' })
    } else {
      await db.risks.add({ ...data, id: crypto.randomUUID() })
      addNotification({ type: 'success', message: 'Riesgo creado' })
    }
    navigate('/governance/risks')
  }

  const severityColor =
    riskScore >= 15 ? 'text-danger' : riskScore >= 8 ? 'text-warning' : 'text-neutral-60'
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Select
              ...
              onChange={(v) => setFormData({ ...formData, probability: parseInt(v) })}
              options={[
          ...
          <div>
            <Select
              ...
              onChange={(v) => setFormData({ ...formData, impact: parseInt(v) })}
              options={[
        ...
        <div>
          <Select
            ...
            onChange={(v) => setFormData({ ...formData, status: v as RiskStatus })}
            options={[
