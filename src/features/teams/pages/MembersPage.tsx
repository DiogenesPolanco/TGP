...
import { Users, Loader2, X, Edit3, Share2 } from 'lucide-react'
...
export function MembersPage() {
  ...
  const sortedMembers = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      let cmp = 0
      switch (sortKey) {
        case 'name': cmp = a.member.displayName.localeCompare(b.member.displayName); break
        case 'role': cmp = a.member.role.localeCompare(b.member.role); break
        case 'team': cmp = a.team.name.localeCompare(b.team.name); break
        case 'status': cmp = a.member.status.localeCompare(b.member.status); break
        case 'sp': cmp = a.kpis.totalSP - b.kpis.totalSP; break
        case 'efficiency': cmp = a.kpis.efficiencyPct - b.kpis.efficiencyPct; break
        case 'mood': cmp = a.kpis.avgMood - b.kpis.avgMood; break
        case 'attention': cmp = a.kpis.attentionScore - b.kpis.attentionScore; break
      }
  ...
  const filteredKpis = useMemo(() => {
    ...
    return {
      bestPerformer: withSP.length > 0 ? withSP.reduce((a, b) => (a.kpis.efficiencyPct > b.kpis.efficiencyPct ? a : b)) : null,
      worstPerformer: withSP.length > 0 ? withSP.reduce((a, b) => (a.kpis.efficiencyPct < b.kpis.efficiencyPct ? a : b)) : null,
      topSP: withSP.length > 0 ? withSP.reduce((a, b) => (a.kpis.totalSP > b.kpis.totalSP ? a : b)) : null,
      needsAttention: filtered.length > 0 ? filtered.reduce((a, b) => (a.kpis.attentionScore > b.kpis.attentionScore ? a : b)) : null,
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <MembersFilterBar
        ...
        onClearFilters={() => { setFilterTeam(''); setFilterStatus('') }}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-neutral-20 dark:divide-neutral-70">
                {paginatedItems.map(({ member, team, kpis }) => (
                  <tr key={member.id} onClick={() => handleEditMember(member.id, member.displayName, team.id)}
                    ...
                    <td className="px-4 py-3 text-secondary">{roleLabels[member.role] ?? member.role}</td>
                    <td className="px-4 py-3 text-secondary">{team.name}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded font-medium ${statusColors[member.status]}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <div className="w-12 h-1 bg-neutral-20 dark:bg-neutral-70 rounded-full overflow-hidden hidden sm:block">
                          <div className="h-full bg-primary rounded-full" style={{ width: `${Math.min(100, kpis.totalSP)}%` }} />
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${kpis.avgMood >= 7 ? 'bg-success' : kpis.avgMood >= 4 ? 'bg-warning' : 'bg-danger'}`} title={`Animo: ${kpis.avgMood}/10`} />
                        <span className={`font-semibold tabular-nums ${kpis.efficiencyPct >= 75 ? 'text-success' : kpis.efficiencyPct >= 50 ? 'text-warning' : 'text-danger'}`}>{kpis.efficiencyPct}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        <span className={`text-xs font-semibold px-1.5 py-0.5 rounded tabular-nums ${kpis.attentionScore <= 20 ? 'bg-success/10 text-success' : kpis.attentionScore <= 50 ? 'bg-warning/10 text-warning' : 'bg-danger/10 text-danger'}`}>{kpis.attentionScore}</span>
