...
export function PublicEquipmentPage() {
  ...
  if (loading)
    return <div className="p-8 text-center text-muted">Cargando reporte de equipamiento...</div>
            ...
            <>
              <div className="grid grid-cols-4 gap-4">
                ...
                <PublicKpi
                  ...
                  label="Disponibles"
                  value={String(data.metrics.available)}
                  color="text-success"
                />
                <PublicKpi
                  icon={<AlertTriangle size={20} />}
                  label="En Mantención"
                  value={String(data.metrics.maintenance)}
                  color="text-warning"
                />
                <PublicKpi
                  icon={<Wrench size={20} />}
                  label="Tickets Abiertos"
                  value={String(data.metrics.openTickets)}
                  color="text-danger"
                />
              </div>
              ...
              <div className="bg-card rounded-xl border border-boundary p-5 space-y-4">
                ...
                <div className="space-y-3">
                  {Object.entries(data.metrics.byType).map(([type, count]) => {
                    const pct =
                      data.metrics.total > 0 ? Math.round((count / data.metrics.total) * 100) : 0
                    return (
                      <div key={type}>
                  ...
                  {(['excellent', 'good', 'fair', 'poor'] as const).map((cond) => {
                    ...
                    return (
                      <div key={cond}>
                        ...
                        <div className="w-full h-2 bg-neutral-20 dark:bg-neutral-70 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${cond === 'excellent' ? 'bg-success' : cond === 'good' ? 'bg-primary' : cond === 'fair' ? 'bg-warning' : 'bg-danger'}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      </div>
              ...
              <div className="bg-card rounded-xl border border-boundary p-5 space-y-4">
                <h3 className="font-semibold text-neutral-90 dark:text-white">
                  Inventario ({data.equipment.length})
                </h3>
                  ...
                  {(() => {
                    ...
                    return data.equipment.slice(0, 50).map((eq) => {
                      ...
                      return (
                        <div
                          key={eq.id}
                          className="flex items-center justify-between p-3 bg-neutral-5 dark:bg-neutral-85 rounded-lg text-sm"
                        >
                          <div className="min-w-0 flex-1">
                            <span className="font-medium text-secondary">
                              {TYPE_LABELS[eq.type] ?? eq.type}
                            </span>
                            <span className="text-neutral-50 ml-2">
                              {eq.brand} {eq.model}
                            </span>
                            {ticketLabels && (
                              <p
