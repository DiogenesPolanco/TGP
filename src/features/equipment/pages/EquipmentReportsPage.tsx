...
import { Monitor, Wrench, AlertTriangle, Share2, Check, Copy, TrendingDown, CheckCircle, RotateCcw, ExternalLink } from 'lucide-react'

export function EquipmentReportsPage() {
  const navigate = useNavigate()
  const metrics = useLiveQuery(() => getEquipmentMetrics(), []) ?? {
    total: 0, available: 0, assigned: 0, maintenance: 0, retired: 0, obsolete: 0,
    openTickets: 0, warrantyExpiring: 0, byType: {}, byCondition: {},
  }
  ...
  const warrantyExpiring = equipment.filter(
    (e) => e.warrantyExpiry && new Date(e.warrantyExpiry) <= new Date(Date.now() + 90 * 24 * 60 * 60 * 1000) && e.status !== 'retired' && e.status !== 'obsolete',
  )
  const openTickets = tickets.filter((t) => t.status === 'open' || t.status === 'in_progress')

  const assignmentRate = metrics.total > 0 ? Math.round((metrics.assigned / metrics.total) * 100) : 0
  const healthRate = metrics.total > 0
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-neutral-90 dark:text-white">Reportes de Equipamiento</h2>
      ...
      {shareUrl && (
        <div className="bg-card rounded-xl border border-boundary p-4 flex items-center gap-3 max-w-full overflow-hidden">
          ...
          <a href={shareUrl} target="_blank" rel="noopener noreferrer"
            className="flex-1 text-xs bg-primary/5 px-3 py-1.5 rounded-lg text-primary hover:text-primary-dark truncate font-mono min-w-0 hover:underline">{shareUrl}</a>
      ...
      <div className="grid grid-cols-4 gap-4">
        <KpiCard icon={<Monitor size={22} />} label="Total Equipos" value={metrics.total} sub={`${assignmentRate}% asignados`} color="text-primary" />
        <KpiCard icon={<CheckCircle size={22} />} label="Disponibles" value={metrics.available} sub="Para asignar" color="text-success" />
        <KpiCard icon={<TrendingDown size={22} />} label="Salud del Inventario" value={`${healthRate}%`} sub={`${metrics.obsolete} obsoletos`} color={healthRate >= 80 ? 'text-success' : healthRate >= 60 ? 'text-warning' : 'text-danger'} />
        <KpiCard icon={<Wrench size={22} />} label="Tickets Abiertos" value={metrics.openTickets} sub={`${openTickets.filter((t) => t.priority === 'critical' || t.priority === 'high').length} prioritarios`} color={metrics.openTickets > 0 ? 'text-warning' : 'text-success'} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="bg-card rounded-xl border border-boundary p-5 space-y-4">
          <h3 className="text-sm font-semibold text-neutral-90 dark:text-white">Distribución por Tipo</h3>
          {typeData.length === 0 ? (
        ...
        <div className="bg-card rounded-xl border border-boundary p-5 space-y-4">
          <h3 className="text-sm font-semibold text-neutral-90 dark:text-white">Distribución por Condición</h3>
          <div className="space-y-3">
            {(['excellent', 'good', 'fair', 'poor'] as const).map((cond) => {
              const count = metrics.byCondition[cond] ?? 0
              const pct = metrics.total > 0 ? Math.round((count / metrics.total) * 100) : 0
              const colors: Record<string, string> = { excellent: 'bg-success', good: 'bg-primary', fair: 'bg-warning', poor: 'bg-danger' }
              return (
                <div key={cond}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="text-secondary">{cond === 'excellent' ? 'Excelente' : cond === 'good' ? 'Bueno' : cond === 'fair' ? 'Regular' : 'Malo'}</span>
                    <span className="font-semibold text-neutral-90 dark:text-white">{count}</span>
                  </div>
                  <div className="w-full h-2 bg-neutral-20 dark:bg-neutral-70 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${colors[cond]}`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
              )
            })}
      ...
      {warrantyExpiring.length > 0 && (
        <div className="bg-card rounded-xl border border-warning/30 p-5 space-y-3">
          <div className="flex items-center gap-2 text-warning">
            ...
            <h3 className="text-sm font-semibold">Garantías Próximas a Vencer ({warrantyExpiring.length})</h3>
          </div>
            ...
            {warrantyExpiring.slice(0, 10).map((e) => {
              ...
              return (
                <div key={e.id} onClick={() => navigate(`/equipment/${e.id}`)}
                  ...
                  <div>
                    <span className="text-secondary">{EQUIPMENT_TYPE_LABELS[e.type]} - {e.brand} {e.model}</span>
                    {ticketLabels && (
                      <span className="text-[10px] text-warning font-mono ml-2">{ticketLabels}</span>
                    )}
                  </div>
                  <span className="text-neutral-50">{e.warrantyExpiry ? new Date(e.warrantyExpiry).toLocaleDateString('es') : '—'}</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="bg-card rounded-xl border border-boundary p-5 space-y-3">
          ...
          <div className="space-y-2">
            {openTickets.slice(0, 10).map((t) => {
              ...
              return (
                <div key={t.id} onClick={() => navigate(`/equipment/${t.equipmentId}`)}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-secondary font-medium">{e ? `${EQUIPMENT_TYPE_LABELS[e.type]} - ${e.brand}` : t.equipmentId}</span>
                      {t.jiraTicketLink ? (
