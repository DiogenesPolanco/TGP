...
export function EquipmentTicketFormPage() {
  ...
  const equipment = useLiveQuery(
    () => (equipmentId ? db.equipment.get(equipmentId) : undefined),
    [equipmentId],
  )
  ...
  if (isEdit && !existingTicket)
    return <div className="p-6 text-neutral-50">Cargando ticket...</div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification({
        type: 'error',
        message: isEdit ? 'Error al actualizar ticket' : 'Error al crear ticket',
      })
    } finally {
      setSaving(false)
    }
      ...
      >
        <div className="p-6 space-y-6">
          <p className="text-sm text-secondary">
            Equipo:{' '}
            <span className="font-medium text-neutral-90 dark:text-white">{equipmentName}</span>
          </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input
                    type="url"
                    value={jiraTicketLink}
                    onChange={(e) => setJiraTicketLink(e.target.value)}
                    placeholder="https://..."
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-neutral-5 dark:bg-neutral-85 border-t border-boundary">
          <Button
            type="button"
            onClick={() => navigate(`/equipment/${equipmentId}`)}
            className="px-4 py-2 text-sm text-neutral-60 hover:text-neutral-90 transition-colors"
          >
            Cancelar
          </Button>
          <Button
