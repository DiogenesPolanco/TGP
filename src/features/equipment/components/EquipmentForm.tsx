...
import type { EquipmentItem, EquipmentType, EquipmentCondition, EquipmentStatus } from '@/types/domain'
...
export function EquipmentForm({ initial, id }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (status === 'assigned' && !assignedTo) {
      addNotification({ type: 'error', message: 'Debes seleccionar un miembro para asignar el equipo' })
      return
    }
    setSaving(true)
    try {
      const data = {
        type, brand: brand.trim(), model: model.trim(), serialNumber: serialNumber.trim(),
        status, condition,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button onClick={() => navigate('/equipment')} className="p-2 rounded-lg hover:bg-neutral-10 dark:hover:bg-neutral-70 transition-colors">
          <ArrowLeft size={20} className="text-neutral-60" />
        </Button>
        <h2 className="text-2xl font-bold text-neutral-90 dark:text-white">{isEdit ? 'Editar Equipo' : 'Nuevo Equipo'}</h2>
      </div>

      <form onSubmit={handleSubmit} className="bg-card rounded-xl border border-boundary overflow-hidden">
        <div className="p-6 space-y-6">
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-neutral-50 uppercase tracking-wider">Información del Equipo</h3>
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-secondary">Tipo *</label>
                <Select value={type} onChange={(v) => setType(v as EquipmentType)} options={EQUIPMENT_TYPES.map((t) => ({ value: t, label: EQUIPMENT_TYPE_LABELS[t] }))} />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={condition} onChange={(v) => setCondition(v as EquipmentCondition)} options={[
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-neutral-50 uppercase tracking-wider">Asignación</h3>
            {status === 'assigned' ? (
              <div className="space-y-1.5">
              ...
              <p className="text-sm text-neutral-40 italic">Cambia el estado a "Asignado" para seleccionar un miembro.</p>
            )}
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-neutral-50 uppercase tracking-wider">Organización</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-secondary flex items-center gap-1.5">
                  <Building2 size={14} />
                ...
                <Select value={businessUnitId} onChange={setBusinessUnitId} options={[
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-neutral-50 uppercase tracking-wider">Garantía y Compra</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-neutral-50 uppercase tracking-wider">Notas</h3>
            <RichTextEditor
              value={notes}
              onChange={setNotes}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-neutral-5 dark:bg-neutral-85 border-t border-boundary">
          <Button type="button" onClick={() => navigate('/equipment')} className="px-4 py-2 text-sm text-neutral-60 hover:text-neutral-90 dark:hover:text-white transition-colors">
            Cancelar
          </Button>
          <Button type="submit" disabled={saving || !brand.trim() || !model.trim() || !serialNumber.trim()}
