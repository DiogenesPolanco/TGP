      ...
      >
        <div className="max-w-[1120px] mx-auto flex items-center justify-between px-6 h-14">
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono text-[11px] font-bold uppercase tracking-wider" style={{ color: '#e8edf5' }}>TGP</span>
            <span className="font-mono text-[10px]" style={{ color: '#4a5568' }}>/</span>
            <span className="font-mono text-[10px] tracking-wider" style={{ color: '#6b7a99' }}>Documentación</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1120px] mx-auto px-6 py-10 flex gap-10">
        ...
        <aside ref={tocRef} className="hidden md:block w-56 shrink-0">
          <nav className="sticky top-24 space-y-1">
            <div className="font-mono text-[10px] uppercase tracking-widest mb-4" style={{ color: '#4a5568' }}>
              En esta página
            </div>
            {SECTIONS.map((s) => (
              <a
        ...
        <div className="md:hidden mb-6 w-full">
          ...
          >
            ...
            <ChevronDown size={14} className={`transition-transform ${mobileTocOpen ? 'rotate-180' : ''}`} />
          </button>
          {mobileTocOpen && (
            <div className="mt-1 rounded-sm overflow-hidden border" style={{ borderColor: 'rgba(0,255,136,0.06)' }}>
              {SECTIONS.map((s) => (
                <a
                  key={s.id}
                  href={`#${s.id}`}
        ...
        <main className="flex-1 min-w-0 max-w-[720px]">
          ...
          <section id="primeros-pasos" className="mb-20 scroll-mt-20">
            ...
            <Body>
              TGP (<Green>T</Green>echnology <Green>G</Green>overnance <Green>P</Green>latform) es una plataforma
              de gobierno tecnológico 100% cliente-side. Unifica en un solo lugar el inventario de aplicaciones,
              vulnerabilidades, riesgos, OKRs, métricas DORA, seguimiento de ejecución y más — todo sin depender
              de un backend externo.
            </Body>
            ...
            <Body>
              La aplicación sigue una arquitectura <strong>feature-first</strong> con separación por dominio.
              Cada feature encapsula páginas, componentes, hooks y servicios propios. La capa de datos unificada
              (Dexie.js repos) vive en <InlineCode>src/services/</InlineCode> y expone operaciones CRUD reactivas
            ...
            <Body>
              ...
              permite desarrollar, testear y eliminar features de forma independiente. El código compartido
              entre features vive en <InlineCode>src/components/</InlineCode> (UI atómica) y
            ...
            <Body>
              ...
              vendor lock-in y mantiene el bundle size mínimo (~150 KB gzip sin datos). Los componentes
            ...
            <Body>
              El build genera archivos estáticos en <InlineCode>dist/</InlineCode>. No necesita Node.js, SSR,
              ni base de datos. Puedes servirlos con cualquier servidor HTTP estático.
            </Body>
            ...
            <Body>
              TGP expone configuración vía variables <InlineCode>VITE_*</InlineCode> (estándar Vite).
              Son opcionales — la app funciona sin ninguna:
            </Body>
            <Table
              rows={[
            ...
            <Body>Referencia rápida de los comandos disponibles en <InlineCode>package.json</InlineCode>:</Body>
            <Table
              rows={[
                ['Comando', 'Acción'],
            ...
            <Body>
              La función <InlineCode>useDemoData()</InlineCode> en <InlineCode>src/hooks/useDemoData.ts</InlineCode>
              popula 25+ tablas con un dataset representativo. Los datos se generan con UUIDs determinísticos
              para evitar duplicados en re-ejecuciones. Incluye:
            </Body>
            <BulletList>
    ...
    'bu-financiero': {
      ...
      thiDelivery: 15, thiCostos: 5            // -5 y -0
            ...
            <Body>
              ...
              aplica primero los pesos globales y luego reemplaza con overrides si existen para la BU
              de la aplicación. Esto permite que diferentes unidades de negocio tengan ponderaciones
            ...
            <Body>
              Un patrón clave de herencia: vulnerabilidades, riesgos, incidentes y hallazgos vinculados
              a un microservicio se heredan automáticamente a la aplicación padre. Esto evita duplicación
              y garantiza visibilidad completa desde el nivel de aplicación.
            </Body>

            <SubSection>Búsqueda y filtros</SubSection>
            <Body>
              La búsqueda usa índices compuestos de Dexie.js sobre los campos <InlineCode>name</InlineCode>,
              <InlineCode> description</InlineCode>, <InlineCode>businessUnitId</InlineCode>. Los filtros
              se aplican con <InlineCode>where().and()</InlineCode> sobre colecciones indexadas, con
            ...
            <Body>
              ...
              duplicados por <InlineCode>name + businessUnitId</InlineCode>, y ejecuta upsert transaccional
              en una sola operación Dexie.js.
            </Body>
            ...
            />
            <Body>
              Cada vulnerabilidad tiene un flujo de estados: <InlineCode>abierta → en_análisis → en_remediación → verificada → cerrada</InlineCode>.
              El SLA se trackea desde <InlineCode>createdAt</InlineCode> y cambia a color según el tiempo
              restante. El servicio de escalamiento en <InlineCode>src/features/execution/services/escalationService.ts</InlineCode>
            ...
            <Body>
              ...
              Cada métrica se evalúa independientemente y el equipo recibe un nivel general igual
              a su peor métrica (el eslabón más débil):
            </Body>
            <CodeBlock>
...
Status:
  ...
  progress < 40%   → behind`}
            ...
            <Body>
              Los OKRs se vinculan con la ejecución operativa mediante dos mecanismos:
            </Body>
            <CodeBlock>
...
3. Reporte de contribución
   ...
   "Actividad: Migrar servidores (contribuye a: Q3-2026-1: Reducir deuda técnica)"`}
            ...
            <Body>
              Las tecnologías registradas se comparan contra la API de <InlineCode>endoflife.date</InlineCode>
              para determinar su estado de soporte. La sincronización se ejecuta bajo demanda desde la
              vista de obsolescencia y cachea los resultados en IndexedDB por 24 horas.
            </Body>
            <CodeBlock>
            ...
            <Body>
              El mapa renderiza un grafo dirigido donde los nodos son tecnologías y las aristas representan
              dependencias entre aplicaciones. Los nodos se colorean según su estado (activo, security-only,
              EOL) y el tamaño refleja la cantidad de aplicaciones impactadas.
            </Body>
            ...
            <Body>
              GobIA soporta 4 proveedores de IA. Todos implementan la interfaz unificada:
            </Body>
            <CodeBlock>
            ...
            <Body>
              ...
              <InlineCode> src/features/ai/tools/registry.ts</InlineCode> y cada uno expone un schema
              Zod, un handler asíncrono y una descripción para el LLM.
            </Body>
            <CodeBlock>
            ...
            <Body>
              ...
              inyecta en el contexto del mensaje al LLM. El sistema multi-turno mantiene el historial
              de conversación en memoria (store Zustand) para respuestas contextualizadas.
            </Body>
...
6. Actualización de historial
   ...
   para mantener coherencia multi-turno.`}
            ...
            <Body>
              ...
              (100k+ registros), las queries a IndexedDB pueden superar los 50 ms — GobIA muestra
              un indicador de escritura ("GobIA está pensando...") mientras se completan.
            </Body>
          </section>
          ...
          <section id="integraciones" className="mb-20 scroll-mt-20">
            ...
            <Body>
              ...
              <InlineCode> equipment_tickets</InlineCode>. La integración es unidireccional:
              TGP almacena la referencia y provee un enlace directo al issue en Jira desde la vista
...
interface AzureConfig {
  ...
  sasToken: string        // Token SAS con permisos: cw (create+write)
            ...
            <Body>
              Los enlaces públicos se cifran en reposo antes de almacenarse. El proceso:
            </Body>
            <CodeBlock>
            ...
            <Body>
              El proceso de importación: 1) Lee el archivo con SheetJS, 2) Valúa cada fila contra
              un schema Zod, 3) Detecta duplicados por <InlineCode>name + businessUnitId</InlineCode>,
              4) Ejecuta upsert en transacción Dexie.js, 5) Reporta resultados (insertados, actualizados, errores).
            </Body>
   ...
   → Una transacción atómica: si falla, rollback completo`}
          ...
          <section id="tecnico" className="mb-20 scroll-mt-20">
            ...
            <Body>
              ...
              <strong> 60% del espacio en disco</strong> (o al menos 1 GB). TGP con uso típico
              ocupa entre 5-50 MB. Para monitorear el uso actual:
            </Body>
            <CodeBlock>
...
navigator.storage.estimate().then((e) => {
  ...
  console.log('Cuota:', e.quota / 1024 / 1024, 'MB')
            ...
            <Body>
              ...
              En la segunda visita, la app carga completa incluso sin conexión de red (App Shell pattern).
              Para forzar actualización del SW: <InlineCode>DevTools → Application → Service Workers → Update</InlineCode>.
            </Body>
   ...
   → Común en errores de IndexedDB o fetch a APIs`}
          ...
          <section id="troubleshooting" className="mb-20 scroll-mt-20">
            ...
            <TroubleBlock q="Error de CORS al usar GobIA con Ollama local">
              ...
              <TroubleSection label="Debug">
                <DebugCode code={`fetch('http://localhost:11434/api/tags').then(r => r.text()).catch(e => 'Error: ' + e.message)`} />
              </TroubleSection>
            </TroubleBlock>
            ...
            <div
              ...
              style={{ background: 'rgba(0,255,136,0.04)', color: '#4a5568', border: '1px solid rgba(0,255,136,0.06)' }}
            >
              Errores por funcionalidad
            </div>
...
function PageH1({ children }: { children: React.ReactNode }) {
  return <h1 className="font-mono text-2xl font-bold mb-1" style={{ color: '#e8edf5' }}>{children}</h1>
}
function Divider() {
  return <div className="w-12 h-px mb-8" style={{ background: '#00ff88' }} />
}
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h3 className="font-mono text-sm font-semibold mb-3 mt-8 first:mt-0" style={{ color: '#e8edf5' }}>{children}</h3>
}
function SubSection({ children }: { children: React.ReactNode }) {
  return <h4 className="font-mono text-[11px] font-semibold uppercase tracking-wider mb-2 mt-6" style={{ color: '#6b7a99' }}>{children}</h4>
}
function Body({ children }: { children: React.ReactNode }) {
  return <p className="text-sm leading-relaxed mb-4" style={{ color: '#8899bb' }}>{children}</p>
}
function BulletList({ children }: { children: React.ReactNode }) {
  return <ul className="text-sm leading-relaxed mb-6 pl-5 space-y-1.5" style={{ color: '#8899bb', listStyle: 'disc' }}>{children}</ul>
}
function InlineCode({ children }: { children: React.ReactNode }) {
  return (
    <code
      className="font-mono text-xs px-1.5 py-0.5 rounded-sm"
      style={{ background: 'rgba(0,255,136,0.06)', color: '#00ff88', border: '1px solid rgba(0,255,136,0.08)' }}
    >
      {children}
    </code>
  )
...
function CodeBlock({ children }: { children: React.ReactNode }) {
  return (
    <pre
      ...
      style={{ background: 'rgba(0,0,0,0.4)', color: '#b0b8c8', border: '1px solid rgba(0,255,136,0.06)' }}
    >
      <code>{children}</code>
    </pre>
  )
      ...
      <table className="w-full font-mono text-[11px] border-collapse">
        <tbody>
          {rows.map((row, ri) => (
            <tr
              key={ri}
              style={{ borderBottom: ri < rows.length - 1 ? '1px solid rgba(0,255,136,0.04)' : 'none' }}
            >
              {row.map((cell, ci) => (
                <td
                  key={ci}
...
function TroubleBlock({
  ...
  return (
    <div className="pb-6 mb-6" style={{ borderBottom: last ? 'none' : '1px solid rgba(0,255,136,0.06)' }}>
      <div className="flex items-start gap-3">
        <CircleHelp size={14} className="shrink-0 mt-0.5" style={{ color: 'rgba(0,255,136,0.3)' }} />
        <div className="flex-1 min-w-0">
          <h4 className="font-mono text-sm font-semibold mb-3" style={{ color: '#e8edf5' }}>{q}</h4>
          {children}
        </div>
      </div>
    </div>
...
function StepList({ items }: { items: string[] }) {
  return (
    <ol className="font-mono text-xs leading-relaxed space-y-1.5 mb-0 ml-0" style={{ color: '#8899bb' }}>
      {items.map((item, i) => (
...
function BulletListInline({ items }: { items: string[] }) {
  return (
    <ul className="font-mono text-xs leading-relaxed space-y-1 ml-0" style={{ color: '#8899bb' }}>
      {items.map((item, i) => (
        <li key={i} className="flex items-baseline gap-2">
          <span className="shrink-0" style={{ color: 'rgba(0,255,136,0.3)' }}>•</span>
          <span>{item}</span>
        </li>
      ))}
    </ul>
...
function DebugCode({ code }: { code: string }) {
  return (
    <pre
      ...
      style={{ background: 'rgba(0,0,0,0.3)', color: '#b0b8c8', border: '1px solid rgba(0,255,136,0.04)' }}
    >
      <code>{code}</code>
    </pre>
  )
