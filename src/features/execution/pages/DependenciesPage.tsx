  ...
  const columns: Column<Dependency>[] = [
    {
      ...
      render: (d) => (
        <span className="text-sm font-medium text-neutral-90 dark:text-white">
          {getEntityTitle(d.sourceType, d.sourceId)}
        </span>
      ),
    },
    {
      ...
      render: (d) => {
        const colors: Record<string, string> = {
          blocks: 'bg-danger/10 text-danger',
          depends_on: 'bg-warning/10 text-warning',
          related_to: 'bg-info/10 text-info',
        }
        return (
          <span className={`text-xs px-2 py-0.5 rounded-full ${colors[d.relationType]}`}>
            {relationLabel[d.relationType]}
          </span>
        )
      },
    },
    ...
    },
    {
      ...
      render: (d) => (
        <span className="text-sm text-secondary">
          {d.expectedResolutionDate
            ? new Date(d.expectedResolutionDate).toLocaleDateString('es-ES')
            : '—'}
        </span>
      ),
    },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            Gestión de dependencias entre tareas, actividades, planes y compromisos
          </p>
        </div>
      </div>
