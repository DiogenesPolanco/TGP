...
export function CommitmentDetailPage() {
  ...
  const team = useLiveQuery(
    () => (commitment?.teamId ? db.teams.get(commitment.teamId) : undefined),
    [commitment],
  )
  const app = useLiveQuery(
    () => (commitment?.applicationId ? db.applications.get(commitment.applicationId) : undefined),
    [commitment],
  )
  const objective = useLiveQuery(
    () => (commitment?.objectiveId ? db.objectives.get(commitment.objectiveId) : undefined),
    [commitment],
  )
  ...
  if (!commitment || !daysInfo) {
    return (
      ...
      >
        <p className="text-neutral-50">El compromiso no existe o ha sido eliminado.</p>
      </DetailLayout>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Section title="Información General">
          ...
          <dd className="text-sm text-neutral-90 dark:text-white flex-1">
            <HtmlDescription html={commitment.description} full />
          </dd>
        ...
        <Section title="Estado y Fechas">
          ...
          <Field
            ...
            value={new Date(commitment.commitmentDate).toLocaleDateString('es-ES')}
          />
          <div className="flex items-start gap-2">
            <dt className="text-xs font-medium text-neutral-50 uppercase tracking-wider min-w-[100px] pt-0.5">
              Cuenta
            ...
            <dd
              className={`text-sm font-medium ${daysInfo.urgent ? 'text-danger' : 'text-neutral-90 dark:text-white'}`}
            >
              {daysInfo.label}
            </dd>
          </div>
          {commitment.fulfilledAt && (
            <Field
              label="Cumplido"
              value={new Date(commitment.fulfilledAt).toLocaleDateString('es-ES')}
            />
          )}
        </Section>
      </div>
