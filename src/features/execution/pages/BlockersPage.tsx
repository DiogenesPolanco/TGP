...
export function BlockersPage() {
  ...
  const columns: Column<Blocker>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (b) => (
        <span className={`text-xs px-2 py-0.5 rounded-full ${severityColor[b.severity]}`}>
    ...
    },
    {
      ...
      render: (b) => {
        ...
        return (
          <span className="text-sm text-secondary">{labels[b.sourceType] ?? b.sourceType}</span>
        )
      },
    },
    {
      ...
      label: 'Asignado',
      sortable: true,
      render: (b) => <span className="text-sm text-secondary">{b.assigneeId ?? '—'}</span>,
    },
    {
      key: 'actions',
      label: '',
      render: (b) => (
        ...
        >
          <Button
            onClick={() => navigate(`/execution/blockers/${b.id}/edit`)}
            variant="ghost"
            size="sm"
            className="p-1.5"
            title="Editar"
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={() => navigate('/execution/blockers/new')}
