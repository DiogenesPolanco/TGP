...
interface PlanSummary { id: string; title: string; startDate: Date; endDate: Date; status: string; health: string }
...
export function ExecutiveGanttChart({ plans, activities, today, timelineStart, timelineEnd, totalWeeks, weekWidth, totalWidth, onNavigate, onToday }: Props) {
  ...
  return (
    <div className="xl:col-span-3 bg-card rounded-xl border border-boundary shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-boundary">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-neutral-50">{timelineStart.toLocaleDateString('es-ES')} — {timelineEnd.toLocaleDateString('es-ES')}</span>
          <Button onClick={() => onNavigate(-12)} className="p-1 rounded hover:bg-neutral-20 dark:hover:bg-neutral-70 text-neutral-50"><ChevronLeft size={16} /></Button>
          <Button onClick={onToday} className="px-2 py-0.5 text-xs font-medium rounded bg-primary/10 text-primary hover:bg-primary/20 transition-colors">Hoy</Button>
          <Button onClick={() => onNavigate(12)} className="p-1 rounded hover:bg-neutral-20 dark:hover:bg-neutral-70 text-neutral-50"><ChevronRight size={16} /></Button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <div style={{ minWidth: `${totalWidth + 200}px` }}>
          <div className="flex border-b border-boundary sticky top-0 bg-card z-10">
            <div className="w-48 shrink-0 px-4 py-2 text-xs font-semibold text-neutral-50 uppercase tracking-wider border-r border-boundary">Plan</div>
            <div className="flex">
              {Array.from({ length: Math.min(totalWeeks, 24) }).map((_, i) => {
                ...
                return (
                  <div key={i} className={`shrink-0 text-center py-2 text-[10px] font-semibold uppercase tracking-wider border-r border-boundary ${isCurrent ? 'bg-primary/[0.04] text-primary' : 'text-neutral-50'}`} style={{ width: `${weekWidth}px` }}>
          ...
          {plans.length === 0 ? (
            <div className="p-8 text-center text-sm text-neutral-50">No hay planes con los filtros seleccionados</div>
          ) : (
            plans.map((plan) => {
              const planStart = new Date(plan.startDate)
              const planEnd = new Date(plan.endDate)
              const leftPct = totalSpan > 0 ? ((planStart.getTime() - timelineStart.getTime()) / totalSpan) * 100 : 0
              const widthPct = totalSpan > 0 ? ((planEnd.getTime() - planStart.getTime()) / totalSpan) * 100 : 0
              ...
              return (
                <div key={plan.id} className="flex items-center border-b border-boundary hover:bg-neutral-10 dark:hover:bg-neutral-70/30 transition-colors cursor-pointer group" onClick={() => navigate(`/execution/plans/${plan.id}`)}>
                  <div className="w-48 shrink-0 px-4 py-3 border-r border-boundary flex items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className={`text-[10px] font-medium ${isOverdue ? 'text-danger' : (statusConfig[plan.status]?.color ?? 'text-neutral-50')}`}>
                  ...
                  <div className="relative flex-1 h-14" style={{ minWidth: `${totalWidth}px` }}>
                    {today >= timelineStart && today <= timelineEnd && (
                      <div className="absolute top-0 bottom-0 w-0.5 bg-danger/60 z-20" style={{ left: `${((today.getTime() - timelineStart.getTime()) / totalSpan) * 100}%` }}>
                        <div className="w-2 h-2 rounded-full bg-danger absolute -top-1 -left-[3px]" />
                      </div>
                    )}
                    <div className="absolute inset-0">
                      {Array.from({ length: Math.min(totalWeeks, 24) }).map((_, i) => (
                        <div key={i} className="absolute top-0 bottom-0 border-l border-neutral-20/50 dark:border-neutral-70/30" style={{ left: `${((i * weekWidth) / (totalWeeks * weekWidth)) * 100}%` }} />
                      ))}
                    </div>
                    {widthPct > 0 && (
                      <div className="absolute top-1/2 -translate-y-1/2 h-8 rounded-md flex items-center px-2 group-hover:shadow-md transition-shadow overflow-hidden"
                        style={{
                        ...
                        <div className="absolute inset-0 rounded-md opacity-20" style={{ width: `${completedPct}%`, backgroundColor: plan.health === 'red' ? '#FF5630' : plan.health === 'yellow' ? '#FFAB00' : '#36B37E' }} />
                        <span className="relative text-[11px] font-medium text-neutral-90 dark:text-white truncate z-10">{plan.title}</span>
                      </div>
                    )}
                  </div>
                </div>
