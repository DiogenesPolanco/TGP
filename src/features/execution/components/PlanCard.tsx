...
export function PlanCard({ plan, activities, today }: PlanCardProps) {
  ...
  const forecastStatus: 'on_track' | 'at_risk' | 'critical' =
    !totalActivities ? 'on_track' :
    isBehind && forecastGap > 25 ? 'critical' :
    isBehind ? 'at_risk' : 'on_track'
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4 mt-1">
        <div className="min-w-0 flex-1 pr-16">
          <h4 className="text-base font-semibold text-neutral-90 dark:text-white truncate">{plan.title}</h4>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-neutral-20 dark:border-neutral-70">
        ...
        <div className={`flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full border ${
          ...
          {forecastStatus === 'critical' ? <AlertTriangle size={10} /> : forecastStatus === 'at_risk' ? <AlertTriangle size={10} /> : <CheckCircle2 size={10} />}
          <span>
            {forecastStatus === 'critical' ? 'Crítico' : forecastStatus === 'at_risk' ? `-${forecastGap}%` : 'En Ruta'}
          </span>
        </div>
      </div>
    </div>
