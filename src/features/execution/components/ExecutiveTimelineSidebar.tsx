...
interface Milestone {
  date: Date; title: string; planTitle: string; planId: string; type: 'due' | 'end'
}

interface Alert {
  severity: 'critical' | 'warning' | 'info'; message: string; link?: string
}
    ...
    <div className="space-y-4">
        ...
        <div className="divide-y divide-neutral-20 dark:divide-neutral-70">
          {milestones.length === 0 ? (
            ...
            milestones.map((m, i) => {
              ...
              return (
                <div key={i} className="px-4 py-3 cursor-pointer hover:bg-neutral-10 dark:hover:bg-neutral-70/50 transition-colors" onClick={() => navigate(`/execution/plans/${m.planId}`)}>
                  <div className="flex items-center gap-2 mb-0.5">
                    {m.type === 'end' ? <Target size={12} className="text-danger" /> : <Calendar size={12} className="text-warning" />}
      ...
      {alerts.length > 0 && (
        <div className="bg-card rounded-xl border border-boundary shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-neutral-20 dark:divide-neutral-70">
            {alerts.map((a, i) => (
              <div key={i} className={`px-4 py-3 flex items-start gap-2 cursor-pointer hover:bg-neutral-10 dark:hover:bg-neutral-70/50 transition-colors ${a.severity === 'critical' ? 'bg-danger/[0.02]' : a.severity === 'warning' ? 'bg-warning/[0.02]' : ''}`}
                onClick={() => a.link && navigate(a.link)}>
                <AlertTriangle size={14} className={`shrink-0 mt-0.5 ${a.severity === 'critical' ? 'text-danger' : a.severity === 'warning' ? 'text-warning' : 'text-info'}`} />
                <div className="min-w-0 flex-1"><p className="text-xs text-secondary">{a.message}</p></div>
