...
export function ComparePage() {
  ...
  const apps = useMemo(
    () => allApps.filter((a) => selectedIds.includes(a.id)),
    [allApps, selectedIds],
  )
  ...
  const getAppTeams = (appId: string) =>
    allTeams.filter((t) => t.businessUnitId === allApps.find((a) => a.id === appId)?.businessUnitId)
            ...
            <thead>
              <tr className="border-b border-boundary">
                ...
                {apps.map((app) => (
                  <th
                    key={app.id}
                    className="p-4 text-sm font-semibold text-neutral-90 dark:text-white min-w-[180px]"
                  >
      ...
      {apps.length < 2 && selectedIds.length > 0 && (
        <div className="text-center py-12 bg-card rounded-2xl border border-boundary">
          <p className="text-neutral-50">
            Selecciona al menos 2 aplicaciones para ver la comparación
          </p>
        </div>
      )}
    </div>
