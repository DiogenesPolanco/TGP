...
export function IncidentForm({ incident, onClose, onSave }: IncidentFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    respondedAt: incident?.respondedAt
      ? new Date(incident.respondedAt).toISOString().split('T')[0]
      : '',
    resolvedAt: incident?.resolvedAt
      ? new Date(incident.resolvedAt).toISOString().split('T')[0]
      : '',
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, downtimeMinutes: parseInt(e.target.value) || 0 })
                }
                className="w-full px-3 py-2 rounded-lg border border-neutral-30 dark:border-neutral-60 bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
              />
            </div>
