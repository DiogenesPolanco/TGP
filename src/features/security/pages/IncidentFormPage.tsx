...
export function IncidentFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      downtimeMinutes: formData.downtimeMinutes ? Number(formData.downtimeMinutes) : null,
      metadata: incident?.metadata ?? {},
      externalId: incident?.externalId ?? `INC-${crypto.randomUUID().slice(0, 8)}`,
      detectedAt: incident?.detectedAt ?? new Date(),
      respondedAt: incident?.respondedAt ?? null,
    ...
    if (incident) {
      ...
      addNotification({ type: 'success', message: 'Incidente actualizado' })
    } else {
      await db.incidents.add({ ...data, id: crypto.randomUUID() })
      addNotification({ type: 'success', message: 'Incidente creado' })
    }
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <Select
              ...
              onChange={(v) => setFormData({ ...formData, status: v as IncidentStatus })}
              options={[
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                downtimeMinutes: e.target.value ? parseInt(e.target.value) : null,
              })
            }
            className="w-full px-3 py-2 rounded-lg border border-neutral-30 dark:border-neutral-60 bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
