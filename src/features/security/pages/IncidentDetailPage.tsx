...
export function IncidentDetailPage() {
  ...
  const app = useLiveQuery(
    () => (incident?.applicationId ? db.applications.get(incident.applicationId) : undefined),
    [incident],
  )

  // ── Related entities ──
  const rawMsJunction = useLiveQuery(
    () => db.incidentMicroservices.where('incidentId').equals(id!).toArray(),
    [id],
  )
  ...
  if (!incident) {
    return (
      <DetailLayout title="Incidente no encontrado" onBack={() => navigate('/security/incidents')}>
        <p className="text-neutral-50">El incidente no existe o ha sido eliminado.</p>
      </DetailLayout>
    )
  }
    ...
    >
      ...
      {activeTab === 'info' && (
        <div className="space-y-6">
          {/* Hero banner */}
          <div
            className={`rounded-xl border p-5 ${severityBg[incident.severity] || 'bg-neutral-10'}`}
          >
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-xl ${severityIconBg[incident.severity] || 'bg-neutral-40'} flex items-center justify-center shadow-sm`}
                >
              ...
              <div
                className={`px-4 py-2 rounded-lg ${severityColor[incident.severity] || 'bg-neutral-50 text-white'} text-center`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Section title="Métricas" icon={<Activity size={18} />}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <MiniField
                  label="Downtime"
                  value={incident.downtimeMinutes ? `${incident.downtimeMinutes} min` : '—'}
                />
              </div>
            </Section>
          </div>
          ...
          <Section title="Timeline" icon={<Calendar size={18} />}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-neutral-10 dark:bg-neutral-70/40 rounded-lg border border-boundary p-3">
                <p className="text-[10px] font-medium text-neutral-50 uppercase tracking-wider mb-1">
                  Detectado
                </p>
              ...
              <div className="bg-neutral-10 dark:bg-neutral-70/40 rounded-lg border border-boundary p-3">
                ...
                <p className="text-sm font-semibold text-neutral-90 dark:text-white">
                  {incident.respondedAt
                    ? new Date(incident.respondedAt).toLocaleDateString('es-ES', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
              ...
              <div className="bg-neutral-10 dark:bg-neutral-70/40 rounded-lg border border-boundary p-3">
                ...
                <p className="text-sm font-semibold text-neutral-90 dark:text-white">
                  {incident.resolvedAt
                    ? new Date(incident.resolvedAt).toLocaleDateString('es-ES', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
