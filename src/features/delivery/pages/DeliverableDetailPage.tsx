...
const statusColor: Record<DeliverableStatus, string> = {
  pending: 'bg-neutral-10 dark:bg-neutral-70 text-neutral-60',
...
export function DeliverableDetailPage() {
  ...
  const app = useLiveQuery(
    () => (deliverable?.applicationId ? db.applications.get(deliverable.applicationId) : undefined),
    [deliverable],
  )
  const objective = useLiveQuery(
    () => (deliverable?.objectiveId ? db.objectives.get(deliverable.objectiveId) : undefined),
    [deliverable],
  )
  ...
  if (!deliverable) {
    return (
      ...
      >
        <p className="text-neutral-50">El entregable no existe o ha sido eliminado.</p>
      </DetailLayout>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Section title="Información General">
          ...
          <dd className="text-sm text-neutral-90 dark:text-white flex-1">
            <HtmlDescription html={deliverable.description} full />
          </dd>
        ...
        <Section title="Estado">
          <div className="flex items-start gap-2">
            ...
            <dd
              className={`text-sm font-medium px-2 py-0.5 rounded ${statusColor[deliverable.status]}`}
            >
          ...
          {daysLeft !== null && (
            <Field
              ...
              value={daysLeft >= 0 ? `${daysLeft} días` : `Vencido hace ${Math.abs(daysLeft)} días`}
            />
          )}
          <Field
            label="Creado"
            value={new Date(deliverable.createdAt).toLocaleDateString('es-ES')}
          />
        </Section>
      </div>
    </DetailLayout>
