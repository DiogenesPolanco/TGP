...
export function MobilePassphraseGate({ theme, onToggleTheme, passphrase, onPassphraseChange, passError, submitting, onSubmit }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)

  return (
    <div className="min-h-dvh bg-canvas text-default flex flex-col items-center justify-center p-6 relative">
      <button onClick={onToggleTheme} className="absolute top-4 right-4 w-8 h-8 rounded-lg hover:bg-neutral-20 dark:hover:bg-neutral-85 flex items-center justify-center transition-colors" aria-label="Cambiar tema">
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="bg-white dark:bg-neutral-85 rounded-2xl border border-boundary p-5 space-y-4">
          <p className="text-sm text-muted leading-relaxed">
            Este dashboard está protegido con cifrado de extremo a extremo. Ingresa la contraseña configurada en el escritorio.
          </p>
          <div className="space-y-2">
            <input ref={inputRef} type="password" value={passphrase}
              onChange={(e) => { onPassphraseChange(e.target.value) }}
              onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter' && !submitting) onSubmit() }}
          ...
          <button onClick={onSubmit} disabled={!passphrase || submitting}
            ...
            {submitting ? <><div className="w-4 h-4 border-2 border-white/30 dark:border-neutral-90/30 border-t-white dark:border-t-neutral-90 rounded-full animate-spin" /> Desencriptando...</> : 'Desbloquear'}
          </button>
        </div>
