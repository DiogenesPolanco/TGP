...
export function MobileDashboardPage() {
  ...
  async function init() {
    ...
    if (cached) { setData(cached); setPhase('dashboard') }
    ...
    if (!cached) {
      setErrorMsg(manifest ? 'No se pudo descargar el snapshot desde Azure.' : 'Enlace inválido. Genera un nuevo enlace desde el escritorio (Administración → Command Center Móvil).')
      setPhase('error')
    }
  }
  ...
  if (phase === 'loading' && !data) {
    return (
      <div className="min-h-dvh bg-canvas text-default flex flex-col items-center justify-center gap-4 p-6 relative">
        <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="absolute top-4 right-4 w-8 h-8 rounded-lg hover:bg-neutral-20 dark:hover:bg-neutral-85 flex items-center justify-center transition-colors" aria-label="Cambiar tema">
  ...
  if (phase === 'error') {
    return (
      <div className="min-h-dvh bg-canvas text-default flex flex-col items-center justify-center gap-4 p-6 relative">
        ...
        <button onClick={handleRefresh} className="mt-4 px-6 py-3 bg-neutral-90 dark:bg-neutral-10 text-white dark:text-neutral-90 hover:bg-neutral-80 dark:hover:bg-neutral-20 rounded-xl text-sm font-semibold transition-colors">Intentar de nuevo</button>
      </div>
    )
  }
  ...
  if (phase === 'passphrase') {
    return (
      <MobilePassphraseGate
        ...
        onPassphraseChange={(v) => { setPassphrase(v); setPassError(false) }}
        passError={passError}
  ...
  if (!data) {
    return <div className="min-h-dvh bg-canvas flex items-center justify-center"><div className="w-8 h-8 border-2 border-neutral-30 dark:border-neutral-60 border-t-primary rounded-full animate-spin" /></div>
  }
