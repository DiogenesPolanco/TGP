...
import {
  parseManifestFromHash, downloadSnapshotFromManifest,
  getCachedMobileSnapshot, setCachedMobileSnapshot, clearCachedMobileSnapshot,
          ...
          <div className="bg-white dark:bg-neutral-85 rounded-2xl border border-boundary p-5 space-y-4">
            ...
            <div className="space-y-2">
                <input
                  ...
                  disabled={submitting}
                />
                {passError && <p className="text-xs text-danger text-center">Contraseña incorrecta. Intenta de nuevo.</p>}
              </div>
              <button onClick={handleSubmitPassphrase} disabled={!passphrase || submitting}
                ...
                {submitting ? (
                  <><div className="w-4 h-4 border-2 border-white/30 dark:border-neutral-90/30 border-t-white dark:border-t-neutral-90 rounded-full animate-spin" /> Desencriptando...</>
                ) : 'Desbloquear'}
              </button>
          </div>
          <p className="text-xs text-neutral-50 text-center">Datos viajan cifrados desde Azure Blob Storage</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-dvh bg-canvas text-default">
      ...
      {showCacheInfo && (
        <div className="mx-4 mt-3 p-3 bg-white dark:bg-neutral-85 rounded-xl border border-boundary text-xs text-muted space-y-1.5">
          ...
          <p className="flex items-center gap-2">
            <span className="text-neutral-70 dark:text-neutral-50 font-medium">Versión snapshot:</span>
      ...
      <div className="px-4 pt-5 pb-8 space-y-5">
          ...
          <div className="flex items-end gap-4">
            <div className="relative">
              <svg width="120" height="120" viewBox="0 0 120 120" className="shrink-0">
                ...
                <circle cx="60" cy="60" r="52" fill="none" stroke="currentColor" strokeWidth="8"
                  strokeDasharray={`${(data.thi.score / 100) * 326.7} 326.7`} strokeLinecap="round"
                ...
                <text x="60" y="52" textAnchor="middle"
                  ...
                  fontSize="26" fontWeight="700" fontFamily="ui-monospace, monospace">{data.thi.score}</text>
                <text x="60" y="68" textAnchor="middle"
                  className="fill-neutral-50"
                  fontSize="8" fontWeight="500">{data.thi.label}</text>
              </svg>
            </div>
              ...
              {data.alerts.slice(0, 5).map((alert, i) => (
                <div key={i}
                  ...
                  <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${
                    alert.type === 'critical' ? 'bg-danger' : alert.type === 'warning' ? 'bg-warning' : 'bg-info'
                  }`}/>
                  <p className="text-xs font-medium leading-relaxed">{alert.message}</p>
                </div>
              ))}
              {data.alerts.length > 5 && (
                <p className="text-xs text-neutral-50 text-center pt-1">+{data.alerts.length - 5} alertas más</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <MetricCard label="Incidentes P1" value={data.incidents.p1} total={data.incidents.open}
          ...
          <MetricCard label="Bloqueos" value={data.blockers.critical} total={data.blockers.open}
            color={data.blockers.critical > 0 ? 'danger' : data.blockers.open > 0 ? 'warning' : 'success'}
          ...
          <MetricCard label="Vulnerabilidades" value={data.vulnerabilities.critical} total={data.vulnerabilities.high}
          ...
          <MetricCard label="Riesgos" value={data.risks.critical} total={data.risks.high}
              ...
              {data.objectives.items.slice(0, 5).map((okr) => (
                <div key={okr.id}
                  ...
                  <div className="w-full h-1.5 bg-neutral-20 dark:bg-neutral-80 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${okr.progress >= 80 ? 'bg-success' : okr.progress >= 50 ? 'bg-warning' : 'bg-danger'}`}
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="w-full h-1 bg-neutral-20 dark:bg-neutral-80 rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${score >= 70 ? 'bg-success' : score >= 50 ? 'bg-warning' : 'bg-danger'}`}
          style={{ width: `${score}%` }}/>
      </div>
    </div>
  )
      ...
      <div className="flex-1 min-w-0">
        ...
        {total > 0 && (
          <div className="w-full h-1 bg-neutral-20 dark:bg-neutral-80 rounded-full overflow-hidden mt-1.5">
            <div className={`h-full rounded-full ${color === 'danger' ? 'bg-danger' : color === 'warning' ? 'bg-warning' : 'bg-success'}`}
              style={{ width: `${pct}%` }}/>
          </div>
        )}
      </div>
