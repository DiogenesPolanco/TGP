...
function MetricCard({
  ...
  return (
    <div className="bg-white dark:bg-neutral-85 rounded-2xl border border-boundary p-4">
      <div
        className={`flex items-center gap-2 mb-3 p-1.5 rounded-lg w-fit ${color === 'danger' ? 'bg-danger/5 text-danger' : color === 'warning' ? 'bg-warning/5 text-warning' : 'bg-success/5 text-success'}`}
      >
        {icon}
      </div>
      <p className="text-xl font-bold text-default font-mono">
        {value}
        <span className="text-sm font-medium text-neutral-50">/{total}</span>
      </p>
...
function StatusRow({
  ...
  return (
    <div className="flex items-center gap-3 bg-white dark:bg-neutral-85 rounded-xl border border-boundary px-4 py-3">
      <div
        className={`w-2 h-2 rounded-full shrink-0 ${color === 'danger' ? 'bg-danger' : color === 'warning' ? 'bg-warning' : 'bg-success'}`}
      />
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted font-medium">{label}</span>
          <span className="text-xs font-mono text-default">
            {value}/{total}
          </span>
        </div>
        {total > 0 && (
          <div className="w-full h-1 bg-neutral-20 dark:bg-neutral-80 rounded-full overflow-hidden mt-1.5">
            <div
              className={`h-full rounded-full ${color === 'danger' ? 'bg-danger' : color === 'warning' ? 'bg-warning' : 'bg-success'}`}
              style={{ width: `${pct}%` }}
            />
          </div>
        )}
    ...
    <div className="min-h-dvh bg-canvas text-default">
        ...
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="w-8 h-8 rounded-lg hover:bg-neutral-20 dark:hover:bg-neutral-85 flex items-center justify-center transition-colors"
      ...
      {showCacheInfo && (
        <div className="mx-4 mt-3 p-3 bg-white dark:bg-neutral-85 rounded-xl border border-boundary text-xs text-muted space-y-1.5">
          ...
          <p className="flex items-center gap-2">
            <span className="text-neutral-70 dark:text-neutral-50 font-medium">
              Versión snapshot:
            </span>
          ...
          <button
            onClick={onRefresh}
            className="text-primary hover:text-primary-dark dark:hover:text-primary font-medium transition-colors"
          >
            Refrescar datos
          </button>
        </div>
      ...
      <div className="px-4 pt-5 pb-8 space-y-5">
        <div className="bg-white dark:bg-neutral-85 rounded-2xl border border-boundary p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] font-mono text-muted">
              {data.applications} apps · {data.teams} equipos
            </span>
          </div>
            ...
            <div className="relative">
              <svg width="120" height="120" viewBox="0 0 120 120" className="shrink-0">
                <circle
                  ...
                  stroke="currentColor"
                  strokeWidth="8"
                  className="text-neutral-20 dark:text-neutral-80"
                />
                <circle
                  cx="60"
                  cy="60"
                  ...
                  stroke="currentColor"
                  strokeWidth="8"
                  strokeDasharray={`${(data.thi.score / 100) * 326.7} 326.7`}
                  strokeLinecap="round"
                ...
                <text
                  ...
                  y="52"
                  textAnchor="middle"
                  className="fill-neutral-90 dark:fill-neutral-10"
                  fontSize="26"
                  fontWeight="700"
        ...
        {data.alerts.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-muted bg-neutral-20 dark:bg-neutral-85 px-1.5 py-0.5 rounded-md">
                {data.alerts.length}
              </span>
            </div>
            <div className="space-y-1.5">
              {data.alerts.slice(0, 5).map((alert, i) => (
                <div
                  key={i}
                  className={`flex items-start gap-2.5 px-3.5 py-2.5 rounded-xl border ${alert.type === 'critical' ? 'bg-danger/5 border-danger/10 text-danger' : alert.type === 'warning' ? 'bg-warning/5 border-warning/10 text-warning' : 'bg-info/5 border-info/10 text-info'}`}
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${alert.type === 'critical' ? 'bg-danger' : alert.type === 'warning' ? 'bg-warning' : 'bg-info'}`}
                  />
                  <p className="text-xs font-medium leading-relaxed">{alert.message}</p>
                </div>
              ))}
              {data.alerts.length > 5 && (
                <p className="text-xs text-neutral-50 text-center pt-1">
                  +{data.alerts.length - 5} alertas más
                </p>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <MetricCard
            label="Incidentes P1"
            value={data.incidents.p1}
            total={data.incidents.open}
            color={data.incidents.p1 > 0 ? 'danger' : 'success'}
            icon={
              <svg
                width="14"
                height="14"
          ...
          <MetricCard
            ...
            total={data.blockers.open}
            color={
              data.blockers.critical > 0 ? 'danger' : data.blockers.open > 0 ? 'warning' : 'success'
            }
            icon={
              <svg
                width="14"
          ...
          <MetricCard
            ...
            total={data.vulnerabilities.high}
            color={
          ...
          <MetricCard
            ...
            total={data.risks.high}
            color={data.risks.critical > 0 ? 'danger' : data.risks.high > 0 ? 'warning' : 'success'}
            icon={
              <svg
                width="14"
                height="14"
        ...
        <div className="space-y-2">
          <StatusRow
            label="Planes en riesgo"
            value={data.plans.atRisk}
            total={data.plans.active}
            color={data.plans.atRisk > 0 ? 'warning' : 'success'}
          />
          <StatusRow
            label="Planes vencidos"
            value={data.plans.overdue}
            total={data.plans.active}
            color={data.plans.overdue > 0 ? 'danger' : 'success'}
          />
          <StatusRow
            label="Compromisos vencidos"
            value={data.commitments.overdue}
            total={data.commitments.total}
            color={data.commitments.overdue > 0 ? 'danger' : 'success'}
          />
        </div>
              ...
              {data.objectives.items.slice(0, 5).map((okr) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-medium text-default truncate flex-1 mr-2">
                      {okr.title}
                    </p>
                    <span
                      className={`text-[10px] font-mono ${okr.status === 'on_track' ? 'text-success' : okr.status === 'at_risk' ? 'text-warning' : 'text-danger'}`}
                    >
                      {okr.progress}%
                    </span>
                  </div>
                  <div className="w-full h-1.5 bg-neutral-20 dark:bg-neutral-80 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${okr.progress >= 80 ? 'bg-success' : okr.progress >= 50 ? 'bg-warning' : 'bg-danger'}`}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5">
              {data.blockers.list.map((b) => (
                <div
                  key={b.id}
                  className="flex items-start gap-2.5 px-3.5 py-2.5 bg-danger/5 border border-danger/10 rounded-xl"
                >
        ...
        <div className="text-center text-[10px] text-neutral-50 pt-4 border-t border-neutral-30 dark:border-neutral-80 space-y-1">
          ...
          <p className="text-neutral-60 dark:text-neutral-50">
            Cifrado AES-GCM 256 · Extremo a extremo
          </p>
        </div>
      </div>
    </div>
