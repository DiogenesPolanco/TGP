...
export async function getCandidateTechnologies(
  candidateId: string,
...
export async function createCandidate(
  ...
  await db.transaction(
    'rw',
    db.candidates,
    db.candidateTechnologies,
    db.candidateEvaluations,
    async () => {
    ...
    async () => {
      ...
      if (team) {
        const member: TeamMember = {
          ...
          userPrincipal:
            candidate.email || `${candidate.name.toLowerCase().replace(/\s+/g, '.')}@tgp.demo`,
