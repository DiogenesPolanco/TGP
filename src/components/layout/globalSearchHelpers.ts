...
export function statusLabel(status: string): string {
  const map: Record<string, string> = {
    open: 'Abierto', mitigated: 'Mitigado', accepted: 'Aceptado', closed: 'Cerrado',
    in_progress: 'En Progreso', resolved: 'Resuelto', overdue: 'Vencido', fixed: 'Corregido',
    detected: 'Detectado', acknowledged: 'Reconocido', active: 'Activo', cancelled: 'Cancelado',
    completed: 'Completado', planned: 'Planificado', on_hold: 'En Pausa', pending: 'Pendiente',
    todo: 'Por Hacer', review: 'En Revisión', done: 'Hecho', at_risk: 'En Riesgo',
    breached: 'Incumplido', fulfilled: 'Cumplido', escalated: 'Escalado',
    not_started: 'No Iniciado', on_track: 'En Curso', behind: 'Atrasado', achieved: 'Logrado',
  }
  return map[status] ?? status
}

export function severityLabel(severity: string): string {
  const map: Record<string, string> = { critical: 'Crítica', high: 'Alta', medium: 'Media', low: 'Baja', info: 'Info' }
  return map[severity] ?? severity
}

export function priorityLabel(priority: string): string {
  const map: Record<string, string> = {
    p0: 'P0 - Crítica', p1: 'P1 - Alta', p2: 'P2 - Media', p3: 'P3 - Baja', p4: 'P4 - Mínima',
    critical: 'Crítica', high: 'Alta', medium: 'Media', low: 'Baja',
  }
  return map[priority] ?? priority
}

export interface SearchGroup { category: string; icon: string; results: SearchResult[] }
export interface SearchResult { id: string; label: string; description: string; route: string; badge?: string; badgeColor?: string }

export interface CategoryMeta { key: string; label: string; icon: string; color: string }
...
export function getCategoryMeta(key: string): CategoryMeta {
  return CATEGORIES_DATA.find((c) => c.key === key) ?? { key, label: key, icon: '📄', color: '#6b7280' }
}
...
async function searchEntity(table: any, fields: string[], query: string, category: string): Promise<{ result: SearchResult; score: number }[]> {
  ...
  for (const item of all) {
    ...
    for (const field of fields) {
      ...
      if (value) { const s = score(query, String(value)); if (s > bestScore) bestScore = s }
    }
...
function mapToResult(item: any, category: string): SearchResult {
  const label = item.name ?? item.title ?? item.displayName ?? item.fullName ?? item.label ?? `${category}#${item.id}`
  const descriptions: Record<string, string[]> = {
    ...
    equipment_ticket: [item.description, item.status, item.priority, item.assigneeId].filter(Boolean),
...
export const SEARCH_REGISTRY: Record<string, SearchConfig> = {
  application: { table: db.applications, fields: ['name', 'description', 'ownerName', 'criticality', 'status'], category: 'application', route: '/catalog/applications' },
  microservice: { table: db.microservices, fields: ['name', 'description', 'serviceLevel', 'lifecycleStatus'], category: 'microservice', route: '/catalog/microservices' },
  technology: { table: db.technologies, fields: ['name', 'version', 'category', 'vendor', 'supportStatus'], category: 'technology', route: '/catalog/obsolescence' },
  vulnerability: { table: db.vulnerabilities, fields: ['title', 'description', 'severity', 'status', 'externalId'], category: 'vulnerability', route: '/security/vulnerabilities' },
  incident: { table: db.incidents, fields: ['title', 'description', 'severity', 'status'], category: 'incident', route: '/security/incidents' },
  team: { table: db.teams, fields: ['name', 'description'], category: 'team', route: '/teams' },
  member: { table: db.users, fields: ['displayName', 'email', 'role'], category: 'member', route: '/teams/members' },
  objective: { table: db.objectives, fields: ['title', 'description', 'status', 'type'], category: 'objective', route: '/strategy/objectives' },
  execution_plan: { table: db.plans, fields: ['title', 'description', 'status', 'health'], category: 'execution_plan', route: '/execution/plans' },
  activity: { table: db.activities, fields: ['title', 'description', 'status', 'priority'], category: 'activity', route: '/execution' },
  commitment: { table: db.commitments, fields: ['title', 'description', 'status'], category: 'commitment', route: '/execution/commitments' },
  dependency: { table: db.dependencies, fields: ['description', 'status', 'relationType'], category: 'dependency', route: '/execution/dependencies' },
  blocker: { table: db.blockers, fields: ['title', 'description', 'severity', 'status'], category: 'blocker', route: '/execution/blockers' },
  daily: { table: db.activities, fields: ['title', 'description', 'status'], category: 'daily', route: '/execution/dailies' },
  sprint: { table: db.teamSprints, fields: ['sprintName', 'quarter', 'year'], category: 'sprint', route: '/performance' },
  evaluation: { table: db.candidateEvaluations, fields: ['category', 'points'], category: 'evaluation', route: '/performance/evaluations' },
  candidate: { table: db.candidates, fields: ['name', 'email', 'position', 'status'], category: 'candidate', route: '/recruitment/candidates' },
  technology_ref: { table: db.candidateTechnologies, fields: ['name', 'points'], category: 'technology_ref', route: '/recruitment/technologies' },
  equipment: { table: db.equipment, fields: ['name', 'type', 'assignedTo', 'status'], category: 'equipment', route: '/equipment' },
  equipment_ticket: { table: db.equipmentTickets, fields: ['description', 'status', 'priority', 'assigneeId'], category: 'equipment_ticket', route: '/equipment/tickets' },
  audit_finding: { table: db.auditFindings, fields: ['title', 'description', 'severity', 'status', 'category'], category: 'audit_finding', route: '/security/audit' },
  risk: { table: db.risks, fields: ['title', 'description', 'severity', 'status', 'category'], category: 'risk', route: '/security/risks' },
}
