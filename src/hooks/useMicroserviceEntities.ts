...
export async function associateToMicroservice(
  ...
  const existing = await db[table].where({ [entityIdKey]: entityId, microserviceId }).first()
...
export async function dissociateFromMicroservice(
  ...
  const record = await db[table].where({ [entityIdKey]: entityId, microserviceId }).first()
